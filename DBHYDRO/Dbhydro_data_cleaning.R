...
#
(LoggerID <- Stations_f$DBKey)
if(is.na(Site_code)){
  (Site_code <- Stations_f$Site)
}
#
#
##### Load new data ----
...
Cleaned_df <- Raw_df %>% 
  # Remove revision date, keep qualifier to overwrite as needed
  dplyr::select(-any_of(c("Revision_Date", "Qualifier"))) %>% 
  # Rename value col to Data type, Station
