...
```{r include = FALSE}
# Set your variables
REnd <- as.Date("2023-12-30")  # Report End Date
```
...
RStart <- floor_date(REnd %m-% months(24), unit = "month") 
...
dboWaterQuality <- tbl(con,in_schema("dbo", "SampleEventWQ")) %>%
  collect() %>%
  filter(substring(SampleEventWQID,1,2) == ECode1)
...
WQStats2 <- WQ1 %>% 
  ...
  summarise(SectionTempMean = mean(Temperature, na.rm = TRUE),
            SectionSalMean = mean(Salinity, na.rm = TRUE),
            SalMax = max(Salinity, na.rm = TRUE),
            SalMin = min(Salinity, na.rm = TRUE))

WQStats3 <- inner_join(WQStats, WQStats2, by = c("AnalysisDate" = "AnalysisDate","SectionName" = "SectionName")) %>%
  mutate(TemperatureDiff = TempMean - SectionTempMean,
         SalMeanDiff = SalMean - SectionSalMean,
         SalMaxDiff = SalMean - SalMax,
         SalMinDiff = SalMean - SalMin)
