...
Author <- c("DB") #Change to your name
```
...
pacman::p_load(odbc, DBI, dbplyr,
  ...
  flextable, officer, officedown, magrittr,
  install = TRUE)
#
...
```{r Data Download}
#Load data from the summary file
...
*The following data updates the "CERPMonthlyReportData" [Excel file](//fwrisharedriveprod.file.core.windows.net/fwrisharedrive/FS1/FishBio/Molluscs/Oysters/CERP/Documents/Grant Reports/CERP 4490/CERPMonthlyReportData.xlsx).*
\
\
Here is the data summary output for SLC on the "Monthly CERP data" Excel sheet:
```{r echo=FALSE, results='asis'}
flextable(SLC_Summary) %>% align(align = "center", part = "all") %>% font(fontname = "Arial", part = "all") %>% fontsize(size = 9, part = "all") %>% fontsize(size = 9, part = "header") %>% autofit() %>% padding(padding= 0, part = "all") %>% width(j = 1, width = 0.8) %>% width(j = 2:ncol(SLC_Summary), width = 0.65)
```
\
Here is the data summary output for CRE and CRW on the "Monthly CR data" Excel sheet:
```{r echo=FALSE, results='asis'}
flextable(CR_Summary %>% dplyr::select(AnalysisDate, starts_with("2")) %>% rename_with(~ str_remove(., "2_"))) %>% align(align = "center", part = "all") %>% autofit() %>% font(fontname = "Arial", part = "all") %>% fontsize(size = 9, part = "all") %>% autofit() %>% padding(padding= 0, part = "all") %>% width(j = 1, width = 0.8) %>% width(j = 2:ncol(CR_Summary %>% dplyr::select(AnalysisDate, starts_with("2"))), width = 0.7) %>% set_caption("CR-East")

flextable(CR_Summary %>% dplyr::select(AnalysisDate_B, starts_with("3")) %>% rename_with(~ str_remove(., "3_"))) %>% align(align = "center", part = "all") %>% autofit() %>% font(fontname = "Arial", part = "all") %>% fontsize(size = 9, part = "all") %>%  autofit() %>% padding(padding= 0, part = "all") %>% width(j = 1, width = 0.8) %>% width(j = 2:ncol(CR_Summary %>% dplyr::select(AnalysisDate_B, starts_with("3"))), width = 0.7) %>% set_caption("CR-West")
```
\
\
\
*The following data updates the "CERPMonthlyReportData_Rio_Station (05-Present)" [Excel file](//fwrisharedriveprod.file.core.windows.net/fwrisharedrive/FS1/FishBio/Molluscs/Oysters/CERP/Documents/Grant Reports/CERP 4490/CERPMonthlyReportData_Rio_Station%20(05-Present).xlsx).*
\
\
Here is the data summary output for St. Lucie-Central station 1 on the "SLC_1" Excel sheet:
```{r echo=FALSE, results='asis'}
flextable(SLC1_Summary) %>% align(align = "center", part = "all") %>% font(fontname = "Arial", part = "all") %>% fontsize(size = 9, part = "all") %>% autofit() %>% padding(padding= 0, part = "all") %>% width(j = 1, width = 0.8) %>% width(j = 2:ncol(SLC1_Summary), width = 0.6)
```
