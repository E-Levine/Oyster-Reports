...
SLC_Flow <- readWorkbook("../DBHYDRO/Shared_data/SL_Flow.xlsx", sheet = 'SLESum', detectDates = TRUE, check.names = TRUE)
...
SLC_Flow_2 <- rbind(SLC_Flow %>% dplyr::select(Date, SLESum) %>% mutate(Site = "SLN"),
  ...
  rbind(SLC_Flow %>% dplyr::select(Date, SLESum) %>% mutate(Site = "SLC")) %>% mutate(Station = 4)
#
...
Sal_SLC_2 %>%
  ggplot(aes(RetDate, Salinity)) + 
  geom_point(aes(shape = Station, fill = Station), size = 2.75) + geom_line(aes(color = Station), linewidth = 0.7) +
  geom_area(data = SLC_Flow, aes(Date, SLESum), color = "darkgray")+
  facet_wrap(.~Site, nrow = 5, labeller = as_labeller(c("SLN" = "St. Lucie-North", "SLS" = "St. Lucie-South", "SLC" = "St. Lucie-Central"))) +
  scale_y_continuous(expand = c(0,0), limits = c(0, 40), sec.axis = sec_axis(~.*100)) +
