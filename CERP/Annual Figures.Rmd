...
knit: (function(inputFile, encoding) {
  rmarkdown::render(
    input = inputFile, 
    encoding = encoding,
    output_file = paste0(substr(inputFile, 1, nchar(inputFile)-4), " - ", with(rmarkdown::yaml_front_matter(inputFile), paste(params$Excel_name)), '.docx')
    ) 
      })
---
...
SLC_Flow <- readWorkbook("../DBHYDRO/Shared_data/SL_Flow.xlsx", sheet = 'SLESum', detectDates = TRUE, check.names = TRUE) %>% filter(Date >= Start_date %m-% days(15) & Date <= End_date)
LRE_Flow <- readWorkbook("../DBHYDRO/Shared_data/LX_Flow.xlsx", sheet = 'LOXSum', detectDates = TRUE, check.names = TRUE) %>% filter(Date >= Start_date %m-% days(15) & Date <= End_date)
CRE_Flow <- readWorkbook("../DBHYDRO/Shared_data/CR_Flow.xlsx", sheet = 'S79', detectDates = TRUE, check.names = TRUE) %>% filter(Date >= Start_date %m-% days(15) & Date %m-% days(15) <= End_date)
#Temperature
...
SLC_Flow_2 <- rbind(SLC_Flow %>% dplyr::select(Date, SLESum) %>% mutate(Site = "SLN"), 
  ...
  mutate(Rescale = (SLESum - min(SLESum, na.rm = T))/(max(SLESum, na.rm = T) - min(SLESum, na.rm = T))*(40-0)+0) #Rescale flow to salinity axis limits for plotting
#
LRE_Flow_1.5 <- LRE_Flow %>% dplyr::select(-Analysis_Date, -LOXSum) %>% gather(Site, Flow, -Estuary, -Date) %>% mutate(Site = case_when(Site == "LOX_Flow" ~ "LXN", TRUE ~ "LXS"))
LRE_Flow_2 <- rbind(LRE_Flow_1.5 %>% filter(Site == "LXN") %>% mutate(Rescale = (Flow - min(Flow, na.rm = T))/(max(Flow, na.rm = T) - min(Flow, na.rm = T))*(40-0)+0), #Rescale flow to salinity axis limits for plotting
                    LRE_Flow_1.5 %>% filter(Site == "LXS") %>% mutate(Rescale = (Flow - min(Flow, na.rm = T))/(max(Flow, na.rm = T) - min(Flow, na.rm = T))*(40-0)+0))
#
CRE_Flow_2 <- rbind(CRE_Flow %>% dplyr::select(Date, Flow) %>% mutate(Site = "CRE"), 
                    CRE_Flow %>% dplyr::select(Date, Flow) %>% mutate(Site = "CRW")) %>% 
  mutate(Rescale = (Flow - min(Flow, na.rm = T))/(max(Flow, na.rm = T) - min(Flow, na.rm = T))*(38-0)+0) #Rescale flow to salinity axis limits for plotting
#
#
##Temperature
...
DermoP_SLE <- Dermo_df %>% dplyr::select(contains("SL")) %>% dplyr::select(contains('RetDate')|contains('Pct'))  %>% filter(SLC_RetDate >= Start_date & SLC_RetDate <= End_date) %>% dplyr::select(-SLN_RetDate, -SLS_RetDate)  %>% rename("SL_RetDate" = SLC_RetDate) %>% gather(SiteStation, Value, -SL_RetDate) %>% rename("RetDate" = SL_RetDate) %>% mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("SLN", "SLS", "SLC")), Station = as.factor(substr(SiteStation, 4, 4)), Helper = case_when(is.na(Value) ~ "*", TRUE ~ NA))
#
DermoP_LRE <- Dermo_df %>% dplyr::select(contains("LX")) %>% dplyr::select(contains('RetDate')|contains('Pct'))  %>% filter(LXN_RetDate >= Start_date & LXN_RetDate <= End_date) %>% dplyr::select(-LXS_RetDate)  %>% rename("LX_RetDate" = LXN_RetDate) %>% gather(SiteStation, Value, -LX_RetDate) %>% rename("RetDate" = LX_RetDate) %>% mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("LXN", "LXS")), Station = as.factor(substr(SiteStation, 4, 4)), Helper = case_when(is.na(Value) ~ "*", TRUE ~ NA))
#
DermoP_CRE <- Dermo_df %>% dplyr::select(contains("CR")) %>% dplyr::select(contains('RetDate')|contains('Pct')) %>% filter(CRE_RetDate >= Start_date & CRE_RetDate <= End_date) %>% dplyr::select(-CRW_RetDate) %>%  rename("RetDate" = CRE_RetDate) %>% gather(SiteStation, Value, -RetDate) %>% mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("CRE", "CRW")), Station = as.factor(substr(SiteStation, 4, 4)), Helper = case_when(is.na(Value) ~ "*", TRUE ~ NA))
#
DermoI_SLE <- Dermo_df %>% dplyr::select(contains("SL")) %>% dplyr::select(contains('RetDate')|-contains('Pct')) %>% filter(SLC_RetDate >= Start_date & SLC_RetDate <= End_date) %>% dplyr::select(-SLN_RetDate, -SLS_RetDate)  %>% rename("SL_RetDate" = SLC_RetDate)  %>% gather(SiteStation, Value, -SL_RetDate) %>% mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("SLN", "SLS", "SLC")), Station = as.factor(substr(SiteStation, 4, 4)), Measure = case_when(grepl("Mean", SiteStation) ~ "Mean", grepl("SD", SiteStation) ~ "SD", TRUE ~ NA), SiteStation = substr(SiteStation, 1, 4)) %>%  spread(Measure, Value) %>% mutate(Helper = case_when(is.na(Mean) ~ "*", TRUE ~ NA))
#
DermoI_LRE <- Dermo_df %>% dplyr::select(contains("LX")) %>% dplyr::select(contains('RetDate')|-contains('Pct')) %>% filter(LXN_RetDate >= Start_date & LXN_RetDate <= End_date) %>% dplyr::select(-LXS_RetDate)  %>% rename("LX_RetDate" = LXN_RetDate) %>% gather(SiteStation, Value, -LX_RetDate) %>% mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("LXN", "LXS")), Station = as.factor(substr(SiteStation, 4, 4)), Measure = case_when(grepl("Mean", SiteStation) ~ "Mean", grepl("SD", SiteStation) ~ "SD", TRUE ~ NA), SiteStation = substr(SiteStation, 1, 4)) %>%  spread(Measure, Value) %>% mutate(Helper = case_when(is.na(Mean) ~ "*", TRUE ~ NA))
#
DermoI_CRE <- Dermo_df %>% dplyr::select(contains("CR")) %>% dplyr::select(contains('RetDate')|-contains('Pct')) %>% filter(CRE_RetDate >= Start_date & CRE_RetDate <= End_date) %>% dplyr::select(-CRW_RetDate)  %>% rename("RetDate" = CRE_RetDate) %>% gather(SiteStation, Value, -RetDate) %>% mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("CRE", "CRW")), Station = as.factor(substr(SiteStation, 4, 4)), Measure = case_when(grepl("Mean", SiteStation) ~ "Mean", grepl("SD", SiteStation) ~ "SD", TRUE ~ NA), SiteStation = substr(SiteStation, 1, 4)) %>%  spread(Measure, Value) %>% mutate(Helper = case_when(is.na(Mean) ~ "*", TRUE ~ NA))
#
#
#
####REPRO DATA
names(Repro_df) <- gsub("\\.", "", names(Repro_df))
Repro_SLE <- Repro_df %>% dplyr::select(contains("SL")) %>% filter(SLC_RetDate >= Start_date & SLC_RetDate <= End_date) %>% dplyr::select(-SLN_RetDate, -SLS_RetDate)  %>% rename("RetDate" = SLC_RetDate) %>% gather(SiteStation, Value, -RetDate) %>% mutate(Stage = factor(substr(SiteStation, 5, 7), levels = c("Ind", "Dev", "Rip", "Spe")), Site = factor(substr(SiteStation, 1, 3), levels = c("SLN", "SLS", "SLC")), Helper = case_when(is.na(Value) ~ "*", TRUE ~ NA))
#
Repro_LRE <- Repro_df %>% dplyr::select(contains("LX")) %>% filter(LXN_RetDate >= Start_date & LXN_RetDate <= End_date) %>% dplyr::select(-LXS_RetDate)  %>% rename("RetDate" = LXN_RetDate) %>% gather(SiteStation, Value, -RetDate) %>% mutate(Stage = factor(substr(SiteStation, 5, 7), levels = c("Ind", "Dev", "Rip", "Spe")), Site = factor(substr(SiteStation, 1, 3), levels = c("LXN", "LXS")), Helper = case_when(is.na(Value) ~ "*", TRUE ~ NA))
#
Repro_CRE <- Repro_df %>% dplyr::select(contains("CR")) %>% filter(CRE_RetDate >= Start_date & CRE_RetDate <= End_date) %>% dplyr::select(-CRW_RetDate) %>%  rename("RetDate" = CRE_RetDate) %>% gather(SiteStation, Value, -RetDate) %>% mutate(Stage = factor(substr(SiteStation, 5, 7), levels = c("Ind", "Dev", "Rip", "Spe")), Site = factor(substr(SiteStation, 1, 3), levels = c("CRE", "CRW")), Helper = case_when(is.na(Value) ~ "*", TRUE ~ NA))
#
#
#
####RECRUITMENT DATA
names(Rcrt_df) <- gsub("\\.", "", names(Rcrt_df))
Rcrt_SLE <- Rcrt_df %>% dplyr::select(contains("SL")) %>% filter(SLC_RetDate >= Start_date & SLC_RetDate <= End_date) %>%  dplyr::select(-SLN_RetDate, -SLS_RetDate)  %>% rename("RetDate" = SLC_RetDate) %>% gather(SiteStation, Value, -RetDate) %>% mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("SLN", "SLS", "SLC")), Station = as.factor(substr(SiteStation, 4, 4)), Measure = case_when(grepl("Mean", SiteStation) ~ "Mean", grepl("SD", SiteStation) ~ "SD", TRUE ~ NA), SiteStation = substr(SiteStation, 1, 4)) %>%  spread(Measure, Value) %>% mutate(Helper = case_when(is.na(Mean) ~ "*", TRUE ~ NA))
#
Rcrt_LRE <- Rcrt_df %>% dplyr::select(contains("LX")) %>% filter(LXN_RetDate >= Start_date & LXN_RetDate <= End_date) %>%  dplyr::select(-LXS_RetDate)  %>% rename("RetDate" = LXN_RetDate) %>% gather(SiteStation, Value, -RetDate) %>% mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("LXN", "LXS")), Station = as.factor(substr(SiteStation, 4, 4)), Measure = case_when(grepl("Mean", SiteStation) ~ "Mean", grepl("SD", SiteStation) ~ "SD", TRUE ~ NA), SiteStation = substr(SiteStation, 1, 4)) %>%  spread(Measure, Value) %>% mutate(Helper = case_when(is.na(Mean) ~ "*", TRUE ~ NA))
#
...
#
####CAGE DATA
#If RetSH is NA then no oysters returned. If all cages for month RetSH = NA then no mort bar.
names(Cage_df) <- gsub("\\.", "", names(Cage_df))
Missing_cages <- Cage_df %>% filter(AnalysisDate >= End_date %m-% months(13)) %>% dplyr::select(AnalysisDate, contains("Ret_SHMean")) %>%  gather(Site, Value, -AnalysisDate) %>% mutate(Helper = case_when(is.na(Value) ~ "*", TRUE ~ NA), Site = factor(substr(Site, 1, 3), levels = c("SLC", "LXN", "CRE", "CRW"))) %>% dplyr::select(-Value)
Cage_Mort <- Cage_df %>% filter(AnalysisDate >= End_date %m-% months(13)) %>% dplyr::select(AnalysisDate, contains("Pct")) %>% gather(Site, Value, -AnalysisDate) %>% mutate(Measure = case_when(grepl("Mean", Site) ~ "Mean", grepl("SD", Site) ~ "SD", TRUE ~ NA), Site = factor(substr(Site, 1, 3), levels = c("SLC", "LXN", "CRE", "CRW"))) %>% spread(Measure, Value) %>% left_join(Missing_cages)
Cage_SH <- Cage_df %>% filter(AnalysisDate >= End_date %m-% months(13)) %>% dplyr::select(AnalysisDate, contains("SH")) %>% gather(SiteType, Value, -AnalysisDate) %>% mutate(Measure = case_when(grepl("Mean", SiteType) ~ "Mean", grepl("SD", SiteType) ~ "SD", TRUE ~ NA), Site = factor(substr(SiteType, 1, 3), levels = c("SLC", "LXN", "CRE", "CRW")), Type = substr(SiteType, 5, 7)) %>% dplyr::select(-SiteType) %>% spread(Measure, Value) %>% mutate(Type = factor(Type, levels = c("Dep", "Ret")), Mean = as.numeric(Mean), SD = as.numeric(SD)) %>% left_join(Missing_cages) %>% mutate(Helper = case_when(Type == "Dep" ~ NA, TRUE ~ Helper))
#
#
```
...
#
##Cages
#Missing ret cages: Missing_cages
...
\
`r if(nrow(CageMort_check) > 0) {flextable(CageMort_check) %>% align(align = "center", part = "all")  %>% autofit() %>% font(fontname = "Arial", part = "all")} else {paste("No missing cage mortality data (deployed and retrieved per month).")}`
`r if(nrow(CageSH_check) > 0) {flextable(CageSH_check) %>% align(align = "center", part = "all")  %>% autofit() %>% font(fontname = "Arial", part = "all")} else {paste("No missing cage shell height data (deployed and retrieved per month).")}`
`r if(nrow(Missing_cages %>% filter(Helper == "*")) > 0) {flextable(Missing_cages %>% filter(Helper == "*")) %>% align(align = "center", part = "all")  %>% autofit() %>% font(fontname = "Arial", part = "all")} else {paste("At elast one cage was retrived for each month after deployment.")}`
\
\
\newpage
...
#max((LRE_Flow_2 %>% filter(Site == "LXN"))$Flow) - round to nearest divisible by 4 -1200, 300; 500
#Change Rescale*n to correct flow pattern as needed LXN 1.64, LXS 1.25
ggarrange(
  #LNX
  ggplot() +
    geom_area(data = LRE_Flow_2 %>% filter(Site == "LXN"), aes(Date, Rescale), fill = rgb(156, 156, 156, maxColorValue = 255))+
...
#max(CRE_Flow_2$Flow, na.rm = T) - round to nearest divisible by 4 - 12569 > 13000, 3250
CR_flow_divide <- 13000
ggarrange(
  #CRE plot
  ggplot() +
    ...
    scale_x_date("", expand= c(0,0), date_breaks = "1 month", date_labels = "%b", limits = c(Start_date %m-% days(5), End_date %m-% days(5)))+
    scale_y_continuous("Salinity", expand = c(0,0), sec.axis = sec_axis(~.*(CR_flow_divide/40), name = "Flow (cfs)", breaks = seq(0,CR_flow_divide, CR_flow_divide/4))) +
    coord_cartesian(ylim = c(0, 40))+
...
#
#Check pH range:
if (min(pH_SLC_2$pH, na.rm = T) < 7 & max(pH_SLC_2$pH, na.rm = T) < 9) {
  SLE_pH_limits <- c(round(min(pH_SLC_2$pH, na.rm = T),1), 9)  # Scale min to 9
} else if (min(pH_SLC_2$pH, na.rm = T) > 7 & max(pH_SLC_2$pH, na.rm = T) > 9) {
  SLE_pH_limits <- c(7, round(max(pH_SLC_2$pH, na.rm = T),1))  # Scale 7 to max
} else if (min(pH_SLC_2$pH, na.rm = T) < 7 & max(pH_SLC_2$pH, na.rm = T) > 9) {
  SLE_pH_limits <- c(round(min(pH_SLC_2$pH, na.rm = T),1), round(max(pH_SLC_2$pH, na.rm = T),1))  # Scale 7 to max
} else {
  SLE_pH_limits <- c(7, 9)  # Use the default limits
}
#
ggarrange(
  #SLN plot
  ggplot() +
    ...
    scale_y_continuous("pH", expand = c(0,0), limits = SLE_pH_limits) +
    BaseForm + FacetForm + West_legend + Station_color_E + Station_fill_E + PlotTitle,
  NULL,
  #SLS plot
  ggplot() +
...
#
#Check pH range:
if (min(pH_LRE_2$pH, na.rm = T) < 7 & max(pH_LRE_2$pH, na.rm = T) < 9) {
  LRE_pH_limits <- c(round(min(pH_LRE_2$pH, na.rm = T),1), 9)  # Scale min to 9
} else if (min(pH_LRE_2$pH, na.rm = T) > 7 & max(pH_LRE_2$pH, na.rm = T) > 9) {
  LRE_pH_limits <- c(7, round(max(pH_LRE_2$pH, na.rm = T),1))  # Scale 7 to max
} else if (min(pH_LRE_2$pH, na.rm = T) < 7 & max(pH_LRE_2$pH, na.rm = T) > 9) {
  LRE_pH_limits <- c(round(min(pH_LRE_2$pH, na.rm = T),1), round(max(pH_LRE_2$pH, na.rm = T),1))  # Scale 7 to max
} else {
  LRE_pH_limits <- c(7, 9)  # Use the default limits
}
#
ggarrange(
  #LXN plot
  ggplot() +
   geom_line(data = pH_LRE_2 %>% filter(Site == "LXN"), aes(RetDate, pH, color = Station), linewidth = 0.7) +
    ...
    labs(title = "Loxahatchee-North") +
    scale_x_date("", expand= c(0,0), date_breaks = "1 month", date_labels = "%b", limits = c(End_date %m-% months(12) %m-% days(5), End_date %m-% days(5)))+
    scale_y_continuous("pH", expand = c(0,0), limits = LRE_pH_limits) +
    BaseForm + FacetForm + West_legend + Station_color_E + Station_fill_E + PlotTitle,
  NULL,
  #LXS plot
  ggplot() +
...
#
#Check pH range:
if (min(pH_CRE_2$pH, na.rm = T) < 7 & max(pH_CRE_2$pH, na.rm = T) < 9) {
  CRE_pH_limits <- c(round(min(pH_CRE_2$pH, na.rm = T),1), 9)  # Scale min to 9
} else if (min(pH_CRE_2$pH, na.rm = T) > 7 & max(pH_CRE_2$pH, na.rm = T) > 9) {
  CRE_pH_limits <- c(7, round(max(pH_CRE_2$pH, na.rm = T),1))  # Scale 7 to max
} else if (min(pH_CRE_2$pH, na.rm = T) < 7 & max(pH_CRE_2$pH, na.rm = T) > 9) {
  CRE_pH_limits <- c(round(min(pH_CRE_2$pH, na.rm = T),1), round(max(pH_CRE_2$pH, na.rm = T),1))  # Scale 7 to max
} else {
  CRE_pH_limits <- c(7, 9)  # Use the default limits
}
ggarrange(
  #CRE plot
  ggplot() +
   geom_line(data = pH_CRE_2 %>% filter(Station != "3" & Station != "4"), aes(RetDate, pH, color = Station), linewidth = 0.7) +
    ...
    labs(title = "Caloosahatchee-East") +
    scale_x_date("", expand= c(0,0), date_breaks = "1 month", date_labels = "%b", limits = c(End_date %m-% months(12) %m-% days(5), End_date %m-% days(5)))+
    scale_y_continuous("pH", expand = c(0,0), limits = CRE_pH_limits) +
    BaseForm + FacetForm + West_legend + Station_color_W + Station_fill_W + PlotTitle ,
  NULL,
  #CRW plot
  ggplot() +
...
for(i in seq_along(SLE_sites)){
  ...
  SLE_Live_list[[i]] <- df_temp %>% ggplot(aes(AnalysisDate, Mean, fill = Station)) + 
    ...
    scale_x_date("Survey", expand= c(0,15), labels = Surveys$Survey, breaks = Surveys$AnalysisDate, limits = c(Start_date, End_date %m+% days(5)))+
...
CRE_live_fig <- ggarrange(
  ...
  CRE_Live_list[[2]] + rremove("x.title") + guides(color = guide_legend(override.aes = list(size=2))),
...
CRE_dead_fig <- ggarrange(
  ...
  CRE_Dead_list[[2]] + rremove("x.title") + guides(color = guide_legend(override.aes = list(size=2))),
  nrow = 2, heights = c(1, 1.0), align = "v"  
  )
#
CRE_dead_fig
...
```{r Cage Mortality Figure, fig.cap = "**Figure 38.**  Mean monthly percent mortality (± S.D.) of oysters planted into open cages at Station 2 in St. Lucie-Central, Station 2 in Loxahatchee-North, Iona Cove in Caloosahatchee River-East, and Bird Island in Caloosahatchee River-West. Asterisks indicate when no cages were retrived at a site.", fig.height = 10, fig.width = 8, fig.fullwidth=TRUE}
ggarrange(
  Cage_Mort %>% filter(Site == "SLC") %>%
    mutate(Mean = case_when(Helper == "*" ~ NA_real_, TRUE ~ Mean), SD = case_when(Helper == "*" ~ NA_real_, TRUE ~ SD)) %>%
    ggplot(aes(AnalysisDate, Mean, fill = Site)) + 
    geom_col(position = position_dodge(12), width = 14, color = "black") + 
    geom_errorbar(aes(ymin = Mean, ymax = Mean + SD), color = "black", position = position_dodge(12), width = 11) +
    geom_text(aes(y = 25, label = Helper, color = Site), position = position_dodge(12), size = 10) +
    labs(title = "St. Lucie-Central Station 2") +
    scale_y_continuous("% Mortality", expand = c(0,0), limits = c(0, 100)) +
    scale_x_date("", expand= c(0,0), date_breaks = "1 month", date_labels = "%b", limits = c(End_date %m-% months(12) %m-% days(10), End_date %m-% days(10)))+
    scale_fill_manual(values = Cages_fill) + scale_color_manual(values = Cages_fill) + BaseForm + theme(legend.position = "none") + PlotTitle,
  NULL,
  ...
  Cage_Mort %>% filter(Site == "CRE") %>%
    ...
    geom_errorbar(aes(ymin = Mean, ymax = Mean + SD), color = "black", position = position_dodge(12), width = 11) +
    geom_text(aes(y = 25, label = Helper, color = Site), position = position_dodge(12), size = 10) +
    labs(title = "Caloosahatchee River-East Iona Cove") +
    scale_y_continuous("% Mortality", expand = c(0,0), limits = c(0, 100)) +
    scale_x_date("", expand= c(0,0), date_breaks = "1 month", date_labels = "%b", limits = c(End_date %m-% months(12) %m-% days(10), End_date %m-% days(10)))+
    scale_fill_manual(values = Cages_fill) + scale_color_manual(values = Cages_fill) + BaseForm + theme(legend.position = "none") + PlotTitle,
  NULL,
  Cage_Mort %>% filter(Site == "CRW") %>%
    ...
    geom_errorbar(aes(ymin = Mean, ymax = Mean + SD), color = "black", position = position_dodge(12), width = 11) +
    geom_text(aes(y = 25, label = Helper, color = Site), position = position_dodge(12), size = 10) +
    labs(title = "Caloosahatchee River-West Bird Island") +
    scale_y_continuous("% Mortality", expand = c(0,0), limits = c(0, 100)) +
    scale_x_date("", expand= c(0,0), date_breaks = "1 month", date_labels = "%b", limits = c(End_date %m-% months(12) %m-% days(10), End_date %m-% days(10)))+
    scale_fill_manual(values = Cages_fill) + scale_color_manual(values = Cages_fill) + BaseForm + theme(legend.position = "none") + PlotTitle,
...
```{r Cage SH Figure, fig.cap = "**Figure 39.**  Mean monthly deployed (hatched bars) and retrieved (solid bars) shell height (± S.D.) of oysters planted into open cages at Station 2 in St. Lucie-Central, Station 2 in Loxahatchee-North, Iona Cove in Caloosahatchee River-East, and Bird Island in Caloosahatchee River-West in 2024. Asterisks indicate when no cages were retrived at a site.", fig.height = 10, fig.width = 8, fig.fullwidth=TRUE}
ggarrange(
  Cage_SH %>% filter(Site == "SLC") %>%  
    mutate(Mean = case_when(Helper == "*" & Type == 'Ret' ~ NA_real_, TRUE ~ Mean), SD = case_when(Helper == "*" & Type == 'Ret'  ~ NA_real_, TRUE ~ SD)) %>% 
    ...
    geom_text(aes(y = 25, label = Helper, color = Site), position = position_dodge(18), size = 10) +
    labs(title = "St. Lucie-Central Station 2") +
    scale_y_continuous("Shell Height (mm)", expand = c(0,0), limits = c(0, 100)) +
    scale_x_date("", expand= c(0,0), date_breaks = "1 month", date_labels = "%b", limits = c(End_date %m-% months(12) %m-% days(10), End_date %m-% days(10)))+
    scale_fill_manual(values = Cages_fill) + scale_color_manual(values = Cages_fill) + scale_color_manual(values = Cages_color) + BaseForm + theme(legend.position = "none") + PlotTitle,
  NULL,
