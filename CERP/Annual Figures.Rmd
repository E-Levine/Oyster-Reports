...
ggarrange(
  Cage_Mort %>% filter(Site == "SLC") %>%
    ...
    scale_fill_manual(values = Cages_fill) + BaseForm + theme(legend.position = "none") + PlotTitle,
  NULL,
...
ggarrange(
  Cage_SH %>% filter(Site == "SLC") %>%  
    ...
    scale_x_date("", expand= c(0,0), date_breaks = "1 month", date_labels = "%b", limits = c(End_date %m-% months(12) %m-% days(10), End_date %m-% days(10)))+
    scale_fill_manual(values = Cages_fill) + scale_color_manual(values = Cages_color) + BaseForm + theme(legend.position = "none") + PlotTitle,
  NULL,
