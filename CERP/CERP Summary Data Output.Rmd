...
##Summary by Cage
#Create empty data frame with analysis dates
Station_summary_C <- data.frame(AnalysisDate = unique(Cage_combined$AnalysisDate))
for(c in Cage_reps){
  ...
  #
  #Join with existing data
  Station_summary_C <- left_join(Station_summary_C, left_join(Summary_cSH, Summary_jC, by = "AnalysisDate"), by = "AnalysisDate")
  #Clean columns for selection and output
  Station_summ_C <- Station_summary_C %>% dplyr::select(-contains(c("-R_Ret", "-Y_Ret"))) %>% rename_with(~sub("-B_RetDate", "_RetDate", .x)) %>% mutate(across(where(is.numeric), round, 2)) %>% arrange(AnalysisDate)
}

##Summary by Site
#Create empty data frame with analysis dates
Site_summary_C <- data.frame(AnalysisDate = unique(Cage_combined$AnalysisDate))
for(j in Cage_Sites){
  Summary_jC <- Cage_combined  %>% 
    #Select columns for each param, filter to Site, and group for summary
  ...
  #
  #Join with existing data
  Site_summary_C <- left_join(Site_summary_C, left_join(Summary_cSH, Summary_jC, by = "AnalysisDate"), by = "AnalysisDate")
  #Clean columns for selection and output
...
#
  #Load each sheet and append new data
  SH_Mort_Cage <- readWorkbook("Summary_Data.xlsx", sheet = 'SH Mort Cage', detectDates = TRUE) %>% bind_rows(Station_summ_C)
  SH_Mort_Site <- readWorkbook("Summary_Data.xlsx", sheet = 'SH Mort Site', detectDates = TRUE) %>% bind_rows(Site_summ_C)
...
#Combined table of all parameters
Sedi_combined <- left_join(rbind((hsdbSedi %>% filter(DataStatus == "Proofed")), (dboSedi %>% filter(DataStatus == "Proofed"))) %>%
  #Create date columns 
  mutate(RetDate = as.Date(substring(SampleEventID, 8, 15), format = "%Y%m%d"),
         DeployedDate = as.Date(DeployedDate, format = "%Y-%m-%d"),
         AnalysisDate = as.Date(floor_date(RetDate, unit = "month")),
         ...
         PctOrganic = ((TotalDW-TotalAsh)/TotalDW)*100,
