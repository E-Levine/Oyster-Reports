...
Database <- "Oysters_24 07 17"  #Set the local database to use
...
for(j in Sites){
  for(s in Stages){
    Repro_comb <- Repro_combined %>% mutate(ReproStage = factor(ReproStage, levels = c("4" , "1", "2", "3", "Z"), labels = c("Ind", "Dev", "Rip", "Spe", "Z"))) %>% filter(ReproStage != "Z")
