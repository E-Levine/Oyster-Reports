...
Append_new_data <- c("Rcrt") #Y (All), N (None), WQ (just WQ), Dermo (just Dermo), Rcrt (just Recruitment)
...
Rcrt_out <- Rcrt_combined %>% group_by(StationName, AnalysisDate) %>% summarise(Mean = mean(BottomMonth, na.rm = T)) %>% pivot_wider(names_from = StationName, values_from = Mean)
