...
Database <- "Oysters_24-10-31"  #Set the local database to use
...
for(j in Sites){
  ...
  Site_summ_Re <- Site_summary_Re %>% mutate(across(where(is.numeric), round, 2)) %>% mutate(across(where(is.numeric), ~na_if(.x, Inf))) %>%
    ...
    dplyr::select(-ends_with(c(".y",".x.x"))) %>% rename_with(~sub("RetDate.x", "RetDate", .x)) %>% mutate(across(where(is.numeric), ~replace(., is.na(.), 0)))
}
#
#
  #Load each sheet and append new data
