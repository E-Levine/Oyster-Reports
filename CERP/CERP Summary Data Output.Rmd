...
Append_new_data <- c("N") #Y (All), N (None), WQ (just WQ), 
...
WQ_out_list <- list()
for(w in WQ_params){
  WQ_Out <- WQ_combined %>% group_by(StationName, AnalysisDate) %>% summarise(across(everything(), ~sum(!is.na(.))))
...
```
## Report summary
This data summary was compiled by `r Author` on `r format(Sys.Date(), "%d %B %Y")` and summarizes data for **CERP** and **PBC** sites. Counts of data consider non-NA entries. NA values are considered "0".\
