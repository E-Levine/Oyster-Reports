...
Append_new_data <- c("Cage") #Y (All), N (None), WQ (just WQ), Dermo (just Dermo), Cage, Rcrt (just Recruitment), Repro, Sedi, Srvy
...
#
#
if(Append_new_data == "Y" | Append_new_data == "Cage"){
  #Load each sheet and append new data
  ...
  print("Cage data was not appended within Excel summary file.")
  }
```
...
## Cage summary
Cage data covers sites and stations from `r min(WQ_combined$AnalysisDate) %>% format("%B %Y")` through `r max(WQ_combined$AnalysisDate) %>% format("%B %Y")`. Cage percentages are average mortality per site.\
`r kable(Cage_out, caption = "Cage percent mortatily data")`
...
`r kable(Repro_out, caption = "Repro percent developing & ripe data")`
