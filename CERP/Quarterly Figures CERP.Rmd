...
The following figures are presented for the quarterly report covering `r Start_date %>% format("%B %Y")` thorough `r End_date %>% format("%B %Y")`. Figures cover data beginning in `r (End_date %m-% months(12) %m-% days(5)) %>% format ("%B %Y")`.
\newpage
...
for(i in seq_along(East_sites)){
  ...
  East_Rcrt_list[[i]] <- df_temp %>% ggplot(aes(RetDate, Mean, fill = Station))+ 
    geom_errorbar(aes(ymin = Mean, ymax = Mean + SD, color = Station), position = position_dodge(9), width = 10) +
