...
*Sedimentation rate mean:* (North, South) `r Sedi_pos %>% subset(AnalysisDate >= Start_date & AnalysisDate < End_date) %>% dplyr::select(contains("RateMean")) %>% colMeans() %>% round(2)`
`r Pos_Pcts %>% tab_caption(caption = "Monthly sedimentation percent organic by North v South stations")`
*Percent organic mean:* (North, South) `r Sedi_pos %>% subset(AnalysisDate >= Start_date & AnalysisDate < End_date) %>% dplyr::select(contains("PctOrganicMean")) %>% colMeans() %>% round(2)`
`r Pos_Wts %>% tab_caption(caption = "Monthly sedimentation organic weights by North v South stations")`
*Organic weight mean:* (North, South) `r Sedi_pos %>% subset(AnalysisDate >= Start_date & AnalysisDate < End_date) %>% dplyr::select(contains("OrgWtMean")) %>% colMeans() %>% round(2)`
