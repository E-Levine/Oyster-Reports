...
<p> Monthly CERP live oyster counts: </p>
`r CERP_count` \
<p>Live Counts range: `r CERP_Counts_temp  %>% slice(n()) %>% pull(Value)` - `r CERP_Counts_temp  %>% slice(1) %>% pull(Value)`\
Live Counts mid-range:`r CERP_Counts_temp  %>% slice(n()-1) %>% pull(Value)` - `r CERP_Counts_temp  %>% slice(2) %>% pull(Value)`</p> \
`r CERP_dead` \
<p> Monthly CERP dead ratios: </p>
<p> DeadRatio range: `r CERP_DR_temp  %>% slice(n()) %>% pull(Value)` - `r CERP_DR_temp  %>% slice(1) %>% pull(Value)` \
DeadRatio mid-range:`r CERP_DR_temp  %>% slice(n()-1) %>% pull(Value)` - `r CERP_DR_temp  %>% slice(2) %>% pull(Value)`</p> \
<p> Monthly CERP shell heights: </p>
`r CERP_SH`\
<p>Shell height range: `r CERP_SH_temp  %>% slice(n()) %>% pull(Value)` - `r CERP_SH_temp  %>% slice(1) %>% pull(Value)`\
Shell height mid-range:`r CERP_SH_temp  %>% slice(n()-1) %>% pull(Value)` - `r CERP_SH_temp  %>% slice(2) %>% pull(Value)` </p>\
...
<p> Recruitment range: `r CERP_Rcrt_temp %>% slice(n()) %>% pull(Value)` - `r CERP_Rcrt_temp %>% slice(1) %>% pull(Value)` 
Recruitment mid-range: `r CERP_Rcrt_temp %>% slice(n()-1) %>% pull(Value)` - `r CERP_Rcrt_temp %>% slice(2) %>% pull(Value)` </p>


<h3> `r paste("Repro summary")` </h3>
`r if(!is.null(nrow(CERP_repro))){CERP_repro %>% tab_caption(caption = "Monthly CERP reproductive stages")} else {paste0("Repro data is not available for the quarter specified.")}`
...
<p> Intensity range: `r Dermo_CERP %>% dplyr::select(contains("Mean")) %>% unlist() %>% min(na.rm = T)` - `r Dermo_CERP %>% dplyr::select(contains("Mean")) %>% unlist() %>% max(na.rm = T)`\
`r paste("Intensity categories: 0-Uninfected; 0.5-Very light;  1-Light;  2-Light to moderate;  3-Moderate;  4-Moderate heavy,  5-Heavy")`\
Prevalence range: `r Dermo_CERP %>% dplyr::select(contains("Pct")) %>% unlist() %>% min(na.rm = T)` - `r Dermo_CERP %>% dplyr::select(contains("Pct")) %>% unlist() %>% max(na.rm = T)` </p>
...
<p> Salinity range: `r CERP_Sal_temp %>% slice(n()) %>% pull(Value)` - `r CERP_Sal_temp %>% slice(1) %>% pull(Value)`\
...
<p> Secchi range: `r WQ_Sec_CERP %>% dplyr::select(contains("SP")) %>% unlist() %>% min()` - `r WQ_Sec_CERP %>% dplyr::select(contains("SP")) %>% unlist() %>% max()`\
...
<p> Live Counts range: `r CRE_Counts_temp  %>% slice(n()) %>% pull(Value)` - `r CRE_Counts_temp  %>% slice(1) %>% pull(Value)`\
Live Counts Mid-range: `r CRE_Counts_temp  %>% slice(n()-1) %>% pull(Value)` - `r CRE_Counts_temp  %>% slice(2) %>% pull(Value)`</p>
`r CRE_dead %>% tab_caption(caption = "Monthly CRE dead ratios")`
<p>DeadRatio range: `r CRE_DR_temp  %>% slice(n()) %>% pull(Value)` - `r CRE_DR_temp  %>% slice(1) %>% pull(Value)`\
DeadRatio Mid-range: `r CRE_DR_temp  %>% slice(n()-1) %>% pull(Value)` - `r CRE_DR_temp  %>% slice(2) %>% pull(Value)`</p>
`r CRE_SH %>% tab_caption(caption = "Monthly CRE shell heights")`
<p>Shell height range: `r CRE_SH_temp  %>% slice(n()) %>% pull(Value)` - `r CRE_SH_temp  %>% slice(1) %>% pull(Value)` \
Shell height mid-range: `r CRE_SH_temp  %>% slice(n()-1) %>% pull(Value)` - `r CRE_SH_temp  %>% slice(2) %>% pull(Value)`</p>
...
<p> Recruitment range: `r CRE_Rcrt_temp  %>% slice(n()) %>% pull(Value)` - `r CRE_Rcrt_temp  %>% slice(1) %>% pull(Value)` \
Recruitment mid-range: `r CRE_Rcrt_temp  %>% slice(n()-1) %>% pull(Value)` - `r CRE_Rcrt_temp  %>% slice(2) %>% pull(Value)`</p>


<h3> `r paste("Repro summary")` </h3>
`r if(!is.null(nrow(CRE_repro))){CRE_repro %>% tab_caption(caption = "Monthly CRE reproductive stages")} else {paste0("Repro data is not available for the quarter specified.")}`
...
<p> Intensity range: `r Dermo_CRE %>% dplyr::select(contains("Mean")) %>% unlist() %>% min(na.rm = T)` - `r Dermo_CRE %>% dplyr::select(contains("Mean")) %>% unlist() %>% max(na.rm = T)`\
`r paste("Intensity categories: 0-Uninfected; 0.5-Very light;  1-Light;  2-Light to moderate;  3-Moderate;  4-Moderate heavy,  5-Heavy")`\
Prevalence range: `r Dermo_CRE %>% dplyr::select(contains("Pct")) %>% unlist() %>% min(na.rm = T)` - `r Dermo_CRE %>% dplyr::select(contains("Pct")) %>% unlist() %>% max(na.rm = T)` </p>
...
<p> Salinity range: `r CRE_Sal_temp %>% slice(n()) %>% pull(Value)` - `r CRE_Sal_temp %>% slice(1) %>% pull(Value)`\
...
Secchi range: `r WQ_Sec_CRE %>% dplyr::select(contains("SP")) %>% unlist() %>% min(na.rm = T)` - `r WQ_Sec_CRE %>% dplyr::select(contains("SP")) %>% unlist() %>% max(na.rm = T)`</p>\
...
<p> Live Counts range: `r PBC_Counts_temp %>% slice(n()) %>% pull(Value)` - `r PBC_Counts_temp %>% slice(1) %>% pull(Value)` \
Live counts Mid-range: `r PBC_Counts_temp %>% slice(n()-1) %>% pull(Value)` - `r PBC_Counts_temp %>% slice(2) %>% pull(Value)`</p>
`r PBC_dead %>% tab_caption(caption = "Monthly PBC dead ratios")`
<p>DeadRatio range: `r PBC_DR_temp %>% slice(n()) %>% pull(Value)` - `r PBC_DR_temp %>% slice(1) %>% pull(Value)`\
DeadRatio Mid-range: `r PBC_DR_temp %>% slice(n()-1) %>% pull(Value)` - `r PBC_DR_temp %>% slice(2) %>% pull(Value)`</p>
`r PBC_SH %>% tab_caption(caption = "Monthly PBC shell heights")`
<p>Shell height range: `r PBC_SH_temp %>% slice(n()) %>% pull(Value)` - `r PBC_SH_temp %>% slice(1) %>% pull(Value)` \
Shell height Mid-range: `r PBC_SH_temp %>% slice(n()-1) %>% pull(Value)` - `r PBC_SH_temp %>% slice(2) %>% pull(Value)` </p>
...
<h3> `r paste("Repro summary")` </h3>
`r if(!is.null(nrow(PBC_repro))){PBC_repro %>% tab_caption(caption = "Monthly PBC reproductive stages")} else {paste0("Repro data is not available for the quarter specified.")}`
...
<p> Intensity range: `r Dermo_PBC %>% dplyr::select(contains("Mean")) %>% unlist() %>% min(na.rm = T)` - `r Dermo_PBC %>% dplyr::select(contains("Mean")) %>% unlist() %>% max(na.rm = T)`\
...
Prevalence range: `r Dermo_PBC %>% dplyr::select(contains("Pct")) %>% unlist() %>% min(na.rm = T)` - `r Dermo_PBC %>% dplyr::select(contains("Pct")) %>% unlist() %>% max(na.rm = T)` </p>
...
<p> Recruitment range: `r PBC_Rcrt_temp %>% slice(n()) %>% pull(Value)` - `r PBC_Rcrt_temp %>% slice(1) %>% pull(Value)`\
Recruitment Mid-range: `r PBC_Rcrt_temp %>% slice(n()-1) %>% pull(Value)` - `r PBC_Rcrt_temp %>% slice(2) %>% pull(Value)`</p>
Summary by reef type:
...
<p> Salinity range: `r PBC_Sal_temp %>% slice(n()) %>% pull(Value)` - `r PBC_Sal_temp %>% slice(1) %>% pull(Value)`\
...
<p> Secchi range: `r WQ_Sec_PBC %>% dplyr::select(contains("SP")) %>% unlist() %>% min(na.rm = T)` - `r WQ_Sec_PBC %>% dplyr::select(contains("SP")) %>% unlist() %>% max(na.rm = T)`\
