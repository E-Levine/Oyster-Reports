...
#######File locations sourced to repo Data folder.
...
WQ_combined <- left_join(rbind((hsdbSampleEventWQ %>% filter(DataStatus == "Proofed" | DataStatus == "Completed")), (dboSampleEventWQ %>% filter(DataStatus == "Proofed"))) %>%
  ...
  dplyr::select(AnalysisDate, RetDate, SampleEventWQID, SampleEventID, FixedLocationID, DissolvedOxygen, PercentDissolvedOxygen, pH, Salinity, Secchi, SP, Temperature, CollectionTime, SampleDepth, TurbidityYSI, TurbidityHach, Comments) %>% 
    rename(DO = DissolvedOxygen, Pct = PercentDissolvedOxygen, Sal = Salinity, Temp = Temperature) %>%
...
#
##Raw data output
WQ_raw_data <- WQ_combined %>% 
  dplyr::select(RetDate, Site, StationName, CollectionTime, SampleDepth, Temp, Sal, pH, DO, Pct, Secchi, TurbidityYSI, TurbidityHach) %>% 
  rename("Station" = StationName, "DOPct" = Pct) %>% 
  mutate(RetDate = format(RetDate, "%m/%d/%Y"), 
         Station = as.numeric(substr(Station, nchar(Station), nchar(Station))), 
         CollectionTime = str_c(str_sub(CollectionTime, 1, nchar(CollectionTime) - 2), ":", str_sub(CollectionTime, -2))) %>% 
  mutate(ChlA = "Z", .before = TurbidityYSI) %>% 
  mutate(Year = year(as.Date(RetDate, format = "%m/%d/%Y")), 
         Month = month(as.Date(RetDate, format = "%m/%d/%Y")), .before = "RetDate")
#
if(params$WQ && !params$Raw_data){
  WQ_combined <- WQ_combined %>% dplyr::select(-Secchi)
  rm(WQ_raw_data)
} else if(!params$WQ && params$Raw_data){
    WQ_raw_data <- WQ_raw_data
    rm(WQ_combined)
} else {
    message("Keeping WQ summary and raw data.")
  }
#
#
##Dermo
...
##Dermo raw data
Dermo_raw_data <- Dermo_combined %>% dplyr::select(RetDate, Site, OysterID, StationName, ShellHeight, TotalWeight, ShellWetWeight, DermoMantle, DermoGill, Comments) %>% rename("SampleNumber" = OysterID, "Station" = StationName) %>% mutate(RetDate = format(RetDate, "%m/%d/%Y"), SampleNumber = as.integer(substring(SampleNumber, 10, 11)), Station = as.numeric(substr(Station, nchar(Station), nchar(Station)))) %>% mutate(Year = year(as.Date(RetDate, format = "%m/%d/%Y")), Month = month(as.Date(RetDate, format = "%m/%d/%Y")), .before = "RetDate") %>% arrange(Year, Month, Site, Station, SampleNumber)
#
#
#
#
...
# Create empty fallback tables if objects do not exist
hsdbCage_safe <- if (exists("hsdbCage")) {
  hsdbCage %>% filter(DataStatus %in% c("Proofed", "Completed"))
} else {
  tibble()
}
dboCage_safe <- if (exists("dboCage")) {
  dboCage %>% filter(DataStatus == "Proofed")
} else {
  tibble()
}
#
Cage_combined <- left_join(rbind((hsdbCage_safe %>% filter(DataStatus == "Proofed" | DataStatus == "Completed")), (dboCage_safe %>% filter(DataStatus == "Proofed"))) %>%
  #Create date columns 
  mutate(RetDate = as.Date(substring(SampleEventID, 8, 15), format = "%Y%m%d"),
         AnalysisDate = as.Date(floor_date(RetDate, unit = "month")),
  ...
  dboFixedLocations %>% 
    mutate(StationName = paste0(Estuary, "-", SectionName, StationNumber),
           Site = paste0(Estuary, "-", SectionName)) %>%
    dplyr::select(FixedLocationID, Site, StationName)) %>% 
  pivot_wider(names_from = DataType, values_from = TotalCount) %>% 
  mutate(Retrieved = if("Retrieved" %in% names(.)) Retrieved else NA_real_,
         Deployed  = if("Deployed"  %in% names(.)) Deployed else NA_real_) %>%
  mutate(CageName = paste0(Site, "-", CageID), 
         DeadPct = case_when(
           "Retrieved" %in% names(.) & "Deployed" %in% names(.) ~ 100 - ((Retrieved / Deployed) * 100),
           TRUE ~ NA_real_))
#
...
if(params$Cage && !params$Raw_data){
  ...
  rm(Cage_counts_raw_data, Cage_SH_raw_data)
    ...
    message("Keeping Cage summary and raw data.")
  }
#
#
#
...
##Repro
##Raw data
Repro_raw_data <- left_join(
  rbind((hsdbRepro %>% dplyr::select(-OldSampleNumber) %>% filter(DataStatus == "Proofed" | DataStatus == "Completed")), 
        (dboRepro  %>% filter(DataStatus == "Proofed"))) %>% 
    mutate(RetDate = as.Date(substring(SampleEventID, 8, 15), format = "%Y%m%d"), 
           ReproStage = as.factor(ReproStage)) %>% 
    dplyr::select(RetDate, OysterID, SampleEventID, FixedLocationID, Sex, ReproStage, Parasite, BadSlide, Comments) %>%
    arrange(match(FixedLocationID, c(LocationID_order))), 
  dboFixedLocations %>% mutate(Site = paste0(Estuary, "-", SectionName)) %>% 
    dplyr::select(FixedLocationID, Site, StationNumber)) %>% 
  left_join(Dermo_combined %>% 
              dplyr::select(OysterID, ShellHeight)) %>% 
  mutate(SampleNumber = as.integer(substr(OysterID, nchar(OysterID)-1, nchar(OysterID))), 
         OldStage = case_when(!is.na(Comments) & grepl("OldStage=", Comments) ~ stringr::str_extract(Comments, "(?<=OldStage=)[^;]+"),
                              TRUE ~ "Z"), 
         MSXPresent = "Z", 
         MSXStage = "Z", 
         M_F = case_when(Sex == "M/F" ~ "Yes", 
                         TRUE ~ "No"), 
         BadSlide = case_when(BadSlide == "N" ~ "No", 
                              BadSlide == "Y" ~ "Yes", TRUE ~ "Other")) %>% 
  dplyr::select(RetDate, Site, SampleNumber, StationNumber, ShellHeight, Sex, ReproStage, OldStage, MSXPresent, MSXStage, Parasite, M_F, BadSlide) %>% 
  rename("Station" = StationNumber) %>% 
  mutate(RetDate = format(RetDate, "%m/%d/%Y"), 
         ReproStage = case_when(ReproStage == "M/F" ~ "Z", 
                                TRUE ~ ReproStage)) %>% 
  mutate(Year = year(as.Date(RetDate, format = "%m/%d/%Y")), 
         Month = month(as.Date(RetDate, format = "%m/%d/%Y")), .before = "RetDate") %>% 
  filter(SampleNumber < 16) %>% 
  arrange(Year, Month, Site, Station, SampleNumber)
#
...
Srvy_combined <- left_join(rbind((hsdbSrvy %>% filter(DataStatus == "Proofed" | DataStatus == "Completed")), (dboSrvy %>% filter(DataStatus == "Proofed"))) %>%
  ...
  dboFixedLocations %>% 
    mutate(StationName = paste0(Estuary, "-", SectionName, StationNumber),
           Site = paste0(Estuary, "-", SectionName)) %>%
    dplyr::select(FixedLocationID, Site, StationName))
#Remove CRE1/CRW4 TIG counts
Srvy_combined <- Srvy_combined %>% filter(!(month(RetDate) != 3 & month(RetDate) != 9 & (StationName == "CR-E1" | StationName == "CR-W4")))
#
...
if(params$Srvy && !params$Raw_data){
  ...
  message("Keeping Survey summary and raw data.")  
  }
#
#
```
...
#
#
  #Load each sheet, determine new data, and append data
 DO_Station <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'DO Station', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x)))
DO_sta_new <- Station_summ %>% dplyr::select(contains("DO")) %>% distinct() %>% filter(!`CR-E_RetDate_DO` %in% unique(DO_Station$`CR-E_RetDate_DO`))
#
 DO_Site <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'DO Site', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x))) 
DO_sit_new <- Site_summ %>% dplyr::select(contains("DO")) %>% distinct() %>% filter(!`CR-E_DO_RetDate` %in% unique(DO_Site$`CR-E_DO_RetDate`))
#
 DOPct_Station <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'DO% Station', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x))) 
DOPct_sta_new <- Station_summ %>% dplyr::select(contains("Pct")) %>% distinct()  %>% filter(!`CR-E_RetDate_Pct` %in% unique(DOPct_Station$`CR-E_RetDate_Pct`))
#
 DOPct_Site <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'DO% Site', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x))) 
DOPct_site_new <- Site_summ %>% dplyr::select(contains("Pct"))  %>% distinct()  %>% filter(!`CR-E_Pct_RetDate` %in% unique(DOPct_Site$`CR-E_Pct_RetDate`))
#
 pH_Station <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'pH Station', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x)))
pH_sta_new <- Station_summ %>% dplyr::select(contains("pH")) %>% distinct()  %>% filter(!`CR-E_RetDate_pH` %in% unique(pH_Station$`CR-E_RetDate_pH`))
#
 pH_Site <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'pH Site', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x))) 
pH_sit_new <- Site_summ %>% dplyr::select(contains("pH")) %>% distinct() %>% filter(!`CR-E_pH_RetDate` %in% unique(pH_Site$`CR-E_pH_RetDate`))
#
 Sal_Station <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'Salinity Station', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x)))
Sal_sta_new <- Station_summ %>% dplyr::select(contains("Sal")) %>% distinct() %>% filter(!`CR-E_RetDate_Sal` %in% unique(Sal_Station$`CR-E_RetDate_Sal`))
#
 Sal_Site <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'Salinity Site', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x)))
Sal_sit_new <- Site_summ %>% dplyr::select(contains("Sal")) %>% distinct() %>% filter(!`CR-E_Sal_RetDate` %in% unique(Sal_Site$`CR-E_Sal_RetDate`))
#
 SP_Station <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'Secchi Station', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x)))
SP_sta_new <- Station_summ %>% dplyr::select(contains("SP")) %>% distinct() %>% filter(!`CR-E_RetDate_SP` %in% unique(SP_Station$`CR-E_RetDate_SP`))
#
 SP_Site <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'Secchi Site', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x)))
SP_site_new <- Site_summ %>% dplyr::select(contains("SP")) %>% distinct() %>% filter(!`CR-E_SP_RetDate` %in% unique(SP_Site$`CR-E_SP_RetDate`))
#
 Temp_Station <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'Temp Station', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x)))
Temp_sta_new <- Station_summ %>% dplyr::select(contains("Temp")) %>% distinct() %>% filter(!`CR-E_RetDate_Temp` %in% unique(Temp_Station$`CR-E_RetDate_Temp`))
#
 Temp_Site <- readWorkbook("../Data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'Temp Site', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("RetDate"), ~as.numeric(.x))) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%Y-%m-%d"))) 
Temp_sit_new <- Site_summ %>% dplyr::select(contains("Temp")) %>% distinct() %>% filter(!`CR-E_Temp_RetDate` %in% unique(Temp_Site$`CR-E_Temp_RetDate`))
  ...
  writeData(WQ_wb, "DO Station", DO_sta_new, startRow = nrow(DO_Station)+2, colNames = FALSE)
  writeData(WQ_wb, "DO Site", DO_sit_new, startRow = nrow(DO_Site)+2, colNames = FALSE)
  writeData(WQ_wb, "DO% Station", DOPct_sta_new, startRow = nrow(DOPct_Station)+2, colNames = FALSE)
  writeData(WQ_wb, "DO% Site", DOPct_site_new, startRow = nrow(DOPct_Site)+2, colNames = FALSE)
  writeData(WQ_wb, "pH Station", pH_sta_new, startRow = nrow(pH_Station)+2, colNames = FALSE)
  writeData(WQ_wb, "pH Site", pH_sit_new, startRow = nrow(pH_Site)+2, colNames = FALSE)
  writeData(WQ_wb, "Salinity Station", Sal_sta_new, startRow = nrow(Sal_Station)+2, colNames = FALSE)
  writeData(WQ_wb, "Salinity Site", Sal_sit_new, startRow = nrow(Sal_Site)+2, colNames = FALSE)
  writeData(WQ_wb, "Secchi Station", SP_sta_new, startRow = nrow(SP_Station)+2, colNames = FALSE)
  writeData(WQ_wb, "Secchi Site", SP_site_new, startRow = nrow(SP_Site)+2, colNames = FALSE)
  writeData(WQ_wb, "Temp Station", Temp_sta_new, startRow = nrow(Temp_Station)+2, colNames = FALSE)
  writeData(WQ_wb, "Temp Site", Temp_sit_new, startRow = nrow(Temp_Site)+2, colNames = FALSE)
  #Save workbook
...
#
  ...
  Dermo_Station <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'Dermo Station', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("Date"), ~as.numeric(.x))) 
Dermo_sta_new <- Station_summ_D %>% distinct() %>% filter(!`CR-E_RetDate` %in% unique(Dermo_Station$`CR-E_RetDate`))
#
  Dermo_Site <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'Dermo Site', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("Date"), ~as.numeric(.x))) 
Dermo_sit_new <- Site_summ_D %>% distinct() %>% filter(!`CR-E_RetDate` %in% unique(Dermo_Site$`CR-E_RetDate`))
#
  
  #Load workbook of existing data
  Data_wb <- loadWorkbook("../Data/CERP_PBC_Summary_Data.xlsx")
  #Overwrite each sheet with new data appended to old
  writeData(Data_wb, "Dermo Station", Dermo_sta_new, startRow = nrow(Dermo_Station)+2, colNames = FALSE)
  writeData(Data_wb, "Dermo Site", Dermo_sit_new, startRow = nrow(Dermo_Site)+2, colNames = FALSE)
   #Save workbook
...
#
  #Load each sheet and append new data
  SH_Mort_Cage <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'SH Mort Cage', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("Date"), ~as.numeric(.x))) 
SH_Mort_Cage_new <- Station_summ_C %>% distinct() %>% filter(!`CR-E_RetDate` %in% unique(SH_Mort_Cage$`CR-E_RetDate`))
  SH_Mort_Site <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'SH Mort Site', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("Date"), ~as.numeric(.x))) 
SH_Mort_Site_new <- Site_summ_C %>% distinct() %>% filter(!`CR-E_Dep_SHMean` %in% unique(SH_Mort_Site$`CR-E_Dep_SHMean`))
  
  #Load workbook of existing data
  Data_wb <- loadWorkbook("../Data/CERP_PBC_Summary_Data.xlsx")
  #Overwrite each sheet with new data appended to old
  writeData(Data_wb, "SH Mort Cage", SH_Mort_Cage_new, startRow = nrow(SH_Mort_Cage)+2, colNames = FALSE)
  writeData(Data_wb, "SH Mort Site", SH_Mort_Site_new, startRow = nrow(SH_Mort_Site)+2, colNames = FALSE)
   #Save workbook
  saveWorkbook(Data_wb, "../Data/CERP_PBC_Summary_Data.xlsx", overwrite=TRUE)
...
#
  #Load each sheet and append new data
  Rcrt_Station <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'Rcrt Station', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("Date"), ~as.numeric(.x))) 
Rcrt_sta_new <- Station_summ_R %>% distinct() %>% filter(!`CR-E_RetDate` %in% unique(Rcrt_Station$`CR-E_RetDate`))
  Rcrt_Site <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'Rcrt Site', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("Date"), ~as.numeric(.x))) 
Rcrt_sit_new <- Site_summ_R %>% distinct() %>% filter(!`CR-E_RetDate` %in% unique(Rcrt_Site$`CR-E_RetDate`))
  
  #Load workbook of existing data
  Data_wb <- loadWorkbook("../Data/CERP_PBC_Summary_Data.xlsx")
  #Overwrite each sheet with new data appended to old
  writeData(Data_wb, "Rcrt Station", Rcrt_sta_new, startRow = nrow(Rcrt_Station)+2, colNames = FALSE)
  writeData(Data_wb, "Rcrt Site", Rcrt_sit_new, startRow = nrow(Rcrt_Site)+2, colNames = FALSE)
   #Save workbook
  saveWorkbook(Data_wb, "../Data/CERP_PBC_Summary_Data.xlsx", overwrite=TRUE)
...
#
  #Load each sheet and append new data
  Repro_Site <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'Repro', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains("Date"), ~as.numeric(.x))) 
Repro_sit_new <- Site_summ_Re %>% distinct() %>% filter(!`CR-E_RetDate` %in% unique(Repro_Site$`CR-E_RetDate`))
  
  #Load workbook of existing data
  Data_wb <- loadWorkbook("../Data/CERP_PBC_Summary_Data.xlsx")
  #Overwrite each sheet with new data appended to old
  writeData(Data_wb, "Repro", Repro_sit_new, startRow = nrow(Repro_Site)+2, colNames = FALSE)
   #Save workbook
  saveWorkbook(Data_wb, "../Data/CERP_PBC_Summary_Data.xlsx", overwrite=TRUE)
...
#Combined table of all parameters
Sedi_combined <- left_join(rbind((hsdbSedi %>% filter(DataStatus == "Proofed" | DataStatus == "Completed") %>%
                                    #Sept 2025 data
...
##Sediment raw data
Sedi_raw_data <- Sedi_combined %>%
       mutate(DaysDeployed = as.numeric(RetDate - DeployedDate), 
              Year = year(as.Date(RetDate, format = "%m/%d/%Y")), 
         Month = month(as.Date(RetDate, format = "%m/%d/%Y")), .before = "RetDate") %>%
       dplyr::select(AnalysisDate, Month, Year, RetDate, DaysDeployed, Site, StationName, "Rep" = CupSampleID, TotalDW, OrganicWt, PctOrganic) %>%
       mutate(StationName = sub("-", "", StationName),
              Rep = substr(Rep, 9, 12))
...
#
  #Load each sheet and append new data
  Sedi_Station <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'Sedi Stations', detectDates = TRUE) 
Sedi_sta_new <- Station_summ_D %>% distinct() %>% filter(!`LW-L_RetDate` %in% unique(Sedi_Station$`LW-L_RetDate`))
  Sedi_Site <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'Sedi Position', detectDates = TRUE) 
Sedi_sit_new <- Site_summ_D %>% distinct() %>% filter(!`North_RetDate` %in% unique(Sedi_Site$`North_RetDate`))
  
  #Load workbook of existing data
  Data_wb <- loadWorkbook("../Data/CERP_PBC_Summary_Data.xlsx")
  #Overwrite each sheet with new data appended to old
  writeData(Data_wb, "Sedi Stations", Sedi_sta_new, startRow = nrow(Sedi_Station)+2, colNames = FALSE)
  writeData(Data_wb, "Sedi Position", Sedi_sit_new, startRow = nrow(Sedi_Site)+2, colNames = FALSE)
   #Save workbook
  saveWorkbook(Data_wb, "../Data/CERP_PBC_Summary_Data.xlsx", overwrite=TRUE)
...
#
  #Load each sheet and append new data
  DeadRatio_Station <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'DeadRatio Station', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains(c("Date", "Survey")), ~as.numeric(.x))) 
DeadRatio_sta_new <- Station_summ_S %>% distinct() %>% filter(!Survey %in% unique(DeadRatio_Station$Survey))
#
  DeadRatio_Site <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'DeadRatio Site', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains(c("Date", "Survey")), ~as.numeric(.x))) 
DeadRatio_sit_new <- Site_summ_S %>% distinct() %>% filter(!Survey %in% unique(DeadRatio_Site$Survey))
#
  SH_Station <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'SH Station', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains(c("Date", "Survey")), ~as.numeric(.x)))
SH_sta_new <- Station_summ_SH %>% distinct() %>% filter(!Survey %in% unique(SH_Station$Survey))
#
  SH_Site <- readWorkbook("../Data/CERP_PBC_Summary_Data.xlsx", sheet = 'SH Site', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%Y-%m-%d")), across(!contains(c("Date", "Survey")), ~as.numeric(.x))) 
SH_sit_new <- Site_summ_SH %>% distinct() %>% filter(!Survey %in% unique(SH_Site$Survey))
  
  #Load workbook of existing data
  Data_wb <- loadWorkbook("../Data/CERP_PBC_Summary_Data.xlsx")
  #Overwrite each sheet with new data appended to old
  writeData(Data_wb, "DeadRatio Station", DeadRatio_sta_new, startRow = nrow(DeadRatio_Station)+2, colNames = FALSE)
  writeData(Data_wb, "DeadRatio Site", DeadRatio_sit_new, startRow = nrow(DeadRatio_Site)+2, colNames = FALSE)
  writeData(Data_wb, "SH Station", SH_sta_new, startRow = nrow(SH_Station)+2, colNames = FALSE)
  writeData(Data_wb, "SH Site", SH_sit_new, startRow = nrow(SH_Site)+2, colNames = FALSE)
   #Save workbook
  saveWorkbook(Data_wb, "../Data/CERP_PBC_Summary_Data.xlsx", overwrite=TRUE)
...
#
#Load each sheet and append new data
  WQ_raw <- readWorkbook("../Data/CERP_PBC_Raw_Data.xlsx", sheet = 'WaterQuality', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%m/%d/%Y")), across(!contains(c("RetDate","Site", "CollectionTime", "ChlA")), ~as.numeric(.x))) %>% mutate(RetDate = format(RetDate, "%m/%d/%Y")) 
WQ_new <- WQ_raw_data %>% distinct() %>% arrange(Year, Month, RetDate, Site, Station) %>% filter(!RetDate %in% unique(WQ_raw$RetDate))
#
  Dermo_raw <- readWorkbook("../Data/CERP_PBC_Raw_Data.xlsx", sheet = 'Dermo', detectDates = TRUE) %>% mutate(across(contains("RetDate"), ~as.Date(.x, format = "%m/%d/%Y")), across(!contains(c("RetDate","Site", "Comments")), ~as.numeric(.x))) %>% mutate(RetDate = format(RetDate, "%m/%d/%Y"))
Dermo_new <- Dermo_raw_data %>% distinct() %>% arrange(Year, Month, Site, Station) %>% filter(!RetDate %in% unique(Dermo_raw$RetDate))
#
  Cage_counts_raw <- readWorkbook("../Data/CERP_PBC_Raw_Data.xlsx", sheet = 'CageCounts', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%m/%d/%Y")), across(!contains(c("Date","Site","Cage", "Comments")), ~as.numeric(.x))) %>% mutate(DeployedDate = format(DeployedDate, "%m/%d/%Y"), RetDate = format(RetDate, "%m/%d/%Y"))
Cage_counts_new <- Cage_counts_raw_data %>% distinct() %>% arrange(Year, Month, Site, Station) %>% filter(!RetDate %in% unique(Cage_counts_raw$RetDate))
#
  Cage_SH_raw <- readWorkbook("../Data/CERP_PBC_Raw_Data.xlsx", sheet = 'CageSHs', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%m/%d/%Y")), across(!contains(c("Date","Site","Cage", "ShellHeightID")), ~as.numeric(.x))) %>% mutate(DeployedDate = format(DeployedDate, "%m/%d/%Y"), RetDate = format(RetDate, "%m/%d/%Y"))
Cage_SH_new <- Cage_SH_raw_data %>% distinct() %>% arrange(Year, Month, Site, Cage) %>% filter(!RetDate %in% unique(Cage_SH_raw$RetDate))
#
  Sedi_raw <- readWorkbook("../Data/CERP_PBC_Raw_Data.xlsx", sheet = 'Sediment', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%m/%d/%Y")), across(!contains(c("Date","Site","Rep", "Comments")), ~as.numeric(.x))) %>% mutate(RetDate = format(RetDate, "%m/%d/%Y"))
Sedi_new <- Sedi_raw_data %>% distinct() %>% arrange(Year, Month, Site, StationName, Rep) %>% filter(!RetDate %in% unique(Sedi_raw$RetDate))
#
  Rcrt_raw <- readWorkbook("../Data/CERP_PBC_Raw_Data.xlsx", sheet = 'Rcrt', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%m/%d/%Y")), across(!contains(c("Date","Site","Cage", "Comments")), ~as.numeric(.x))) %>% mutate(RetDate = format(RetDate, "%m/%d/%Y"))
Rcrt_new <- Rcrt_raw_data %>% distinct() %>% arrange(Year, Month, Site, Station, Rep, Shell) %>% filter(!RetDate %in% unique(Rcrt_raw$RetDate))
#
  Repro_raw <- readWorkbook("../Data/CERP_PBC_Raw_Data.xlsx", sheet = 'Repro', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%m/%d/%Y"))) %>% mutate(RetDate = format(RetDate, "%m/%d/%Y")) 
Repro_new <- Repro_raw_data %>% distinct() %>% arrange(RetDate)  %>% arrange(Year, Month) %>% filter(!RetDate %in% unique(Repro_raw$RetDate))
#
  Srvy_counts_raw <- readWorkbook("../Data/CERP_PBC_Raw_Data.xlsx", sheet = 'SrvyCounts', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%m/%d/%Y"))) %>% mutate(RetDate = format(RetDate, "%m/%d/%Y")) 
Srvy_counts_new <- Srvy_counts_raw_data %>% distinct() %>% arrange(Year, Month, Site, Station, QuadratID) %>% filter(!RetDate %in% unique(Srvy_counts_raw$RetDate))
#
  Srvy_SH_raw <- readWorkbook("../Data/CERP_PBC_Raw_Data.xlsx", sheet = 'SrvySHs', detectDates = TRUE) %>% mutate(across(contains("Date"), ~as.Date(.x, format = "%m/%d/%Y"))) %>% mutate(RetDate = format(RetDate, "%m/%d/%Y")) 
Srvy_SH_new <- Srvy_SH_raw_data %>% distinct() %>% arrange(Year, Month, Site, Station, Quadrat) %>% filter(!RetDate %in% unique(Srvy_SH_raw$RetDate))
  
  #Load workbook of existing data
  Raw_data_wb <- loadWorkbook("../Data/CERP_PBC_Raw_Data.xlsx")
  #Overwrite each sheet with new data appended to old
  writeData(Raw_data_wb, sheet = "WaterQuality", WQ_new, startRow = nrow(WQ_raw)+2, colNames = FALSE)
  writeData(Raw_data_wb, "Dermo", Dermo_new, startRow = nrow(Dermo_raw)+2, colNames = FALSE)
  writeData(Raw_data_wb, "CageCounts", Cage_counts_new, startRow = nrow(Cage_counts_raw)+2, colNames = FALSE)
  writeData(Raw_data_wb, "CageSHs", Cage_SH_new, startRow = nrow(Cage_SH_raw)+2, colNames = FALSE)
  writeData(Raw_data_wb, "Sediment", Sedi_new, startRow = nrow(Sedi_raw)+2, colNames = FALSE)
  writeData(Raw_data_wb, "Rcrt", Rcrt_new, startRow = nrow(Rcrt_raw)+2, colNames = FALSE)
  writeData(Raw_data_wb, "Repro", Repro_new, startRow = nrow(Repro_raw)+2, colNames = FALSE)
  writeData(Raw_data_wb, "SrvyCounts", Srvy_counts_new, startRow = nrow(Srvy_counts_raw)+2, colNames = FALSE)
  writeData(Raw_data_wb, "SrvySHs", Srvy_SH_new, startRow = nrow(Srvy_SH_raw)+2, colNames = FALSE)
...
`r if(exists("Cage_out") == TRUE){kable(Cage_out, caption = "Cage percent mortatily data. Cage percentages are average mortality per site.")}`
...
`r if(exists("Srvy_out") == TRUE){kable(Srvy_out, caption = "Survey data. Values provided are mean number of live oysters (per m2).")}`