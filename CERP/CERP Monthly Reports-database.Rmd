...
CR_i <- left_join(
    left_join(
      left_join(CR_WQStats %>% ungroup() %>% subset(StationNumber == i) %>%
                dplyr::select(StationNumber, AnalysisDate, SalMean, SalSD), 
...
CR_Summary <- CR_Summary %>% pivot_wider(names_from = StationNumber, values_from = c(3:13), names_glue = "{StationNumber}_{.value}")
return(CR_Summary)
}
...
## Report summary
This monthly report was compiled by `r Author` on `r format(Sys.Date(), "%d %B %Y")`. and details data for **CERP** sites in **`r MonYr`**. Analysis covers data occurring between *`r ReportStart`* and *`r ReportEnd`* at St. Lucie-Central and at Caloosahatchee River sites.\
...
*	The mean recruitment rate 
`r if((Rcrt_summary %>% filter(Site == "CR-E" & RetDate == min((Rcrt %>% filter(Site == "CR-E"))$Ret.Date) %>% format("%B")))$meanRcrt > (Rcrt_summary %>% filter(Site == "CR-E" & RetDate == max((Rcrt %>% filter(Site == "CR-E"))$Ret.Date) %>% format("%B")))$meanRcrt) {paste("decreased")} else if ((Rcrt_summary %>% filter(Site == "CR-E" & RetDate == min((Rcrt %>% filter(Site == "CR-E"))$Ret.Date) %>% format("%B")))$meanRcrt < (Rcrt_summary %>% filter(Site == "CR-E" & RetDate == max((Rcrt %>% filter(Site == "CR-E"))$Ret.Date) %>% format("%B")))$meanRcrt) {paste("increased")} else {paste("stayed the same")}` 
from 
`r (Rcrt_summary %>% filter(Site == "CR-E" & RetDate == min((Rcrt %>% filter(Site == "CR-E"))$Ret.Date) %>% format("%B")))$meanRcrt` 
spat/shell in 
`r min((Rcrt %>% filter(Site == "CR-E"))$Ret.Date) %>% format("%B")` 
to 
`r (Rcrt_summary %>% filter(Site == "CR-E" & RetDate == max((Rcrt %>% filter(Site == "CR-E"))$Ret.Date) %>% format("%B")))$meanRcrt` 
spat/shell in 
