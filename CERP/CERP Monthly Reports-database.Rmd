...
CR_Summary <- left_join(
  left_join(
    left_join(CR_WQStats %>% ungroup() %>% group_by(StationNumber) %>%
                dplyr::select(StationNumber, AnalysisDate, SalMean, SalSD), 
              CR_RecruitStats %>% ungroup() %>% group_by(StationNumber) %>%
                dplyr::select(StationNumber, AnalysisDate, RcrtMean, RcrtSD)),
    CR_DermoStats %>% ungroup() %>% group_by(StationNumber) %>%
      dplyr::select(StationNumber, AnalysisDate, DermoPct)),
  CR_SurveyStats %>% ungroup() %>% group_by(StationNumber) %>%
    dplyr::select(StationNumber, AnalysisDate, LiveMean:DeadSD)) %>%
                pivot_wider(names_from = StationNumber, values_from = c(3:11), names_glue = "{StationNumber}_{.value}") %>%
  mutate(across(where(is.numeric), round, 2))
```
