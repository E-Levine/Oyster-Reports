...
Database <- "OystersEL_240111"  #Set the local database to use
...
Sedi1 <- union(hsdbSediment, dboSediment) %>% 
  mutate(DeployedDate = as.Date(DeployedDate), 
         ...
         TotalAsh = (AshWeight - TareCrucible)*(1/PortionofSample), 
...
LWL_Srvy_note <- if(Month == "03" | Month == "06" | Month == "09" | Month == "12") {paste0("During the biannual ", if(Month == "03") {paste("spring")} else if(Month == "06") {paste("summer")} else if (Month == "09") {paste("fall")} else {paste("winter")}, " survey there were approximately ", round(LWL_SurveyStats$LiveMean,0), " live and ", round(LWL_SurveyStats$DeadMean,0), " dead oysters per 1 m2 quadrat.")} else {paste("Survey was not conducted this month")}

LWR_Srvy_note <- if(Month == "03" | Month == "06" | Month == "09" | Month == "12") {paste0("During the biannual ", if(Month == "03") {paste("spring")} else if(Month == "06") {paste("summer")} else if (Month == "09") {paste("fall")} else {paste("winter")}, " survey there were approximately ", round(LWR_SurveyStats$LiveMean,0), " live and ", round(LWR_SurveyStats$DeadMean,0), " dead oysters per 1 m2 quadrat.")} else {paste("Survey was not conducted this month")}
...
## Report summary
This monthly report was compiled by `r Author` on `r format(Sys.Date(), "%d %B %Y")` and summarizes data for **Palm Beach County** sites in **`r MonYr`**. Analysis covers data occurring between *`r ReportStart`* and *`r ReportEnd`* at natural reef and restored reef stations.\
...
* `r if(Month == "03" | Month == "06"  | Month == "09" | Month == "12") {paste(LWL_Srvy_note)} else {paste("Survey was not conducted this month")}`
\
...
#Create base table
Sedi_table <- LW_SediStats %>% mutate(Month = format(RetDate, "%B"), Station = paste0(Estuary, SectionName, StationNumber), MonthRate = round(MonthRate, 2), MeanOrganic = round(MeanOrganic, 2)) %>% rename("Monthly rate (g/month)" = MonthRate, "Mean % Organic" = MeanOrganic) %>% mutate(Station = factor(Station, levels = NS_Order)) %>% pivot_wider(id_cols = Station, names_from = Month, values_from = c("Monthly rate (g/month)", "Mean % Organic"), names_glue = "{Month}_{.value}") %>% arrange(Station)


flextable(Sedi_table) %>% separate_header() %>% align(align = "center") %>% 
  hline(i = 3, border = officer::fp_border(style = "dashed")) %>% vline(j = c(1,3), border = officer::fp_border(style = "solid")) %>% autofit() %>% font(fontname = "Arial", part = "all") %>% fontsize(size = 11, part = "all") 
```
