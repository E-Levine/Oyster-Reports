...
```{r table formatting}
#Salinity
WQ_Sal_tab <- WQ_Sal %>% gather(Site_Station, Salinity, -RetDate) %>% mutate(Site_Station = gsub(("_Sal|\\."), "", Site_Station)) %>% mutate(Site = substr(Site_Station, 1, 3), Station = substr(Site_Station, 4, 5)) %>% dplyr::select(-Site_Station) %>% mutate(Site = case_when(Site == "CRE" | Site == "CRW" ~ "CR", TRUE ~ Site)) 
#
(WQ_Sal_out <- WQ_Sal_tab %>% spread(Station, Salinity) %>% gt(groupname_col = "Site") %>% data_color(columns = contains(c("1","2","3","4")), fn = scales::col_numeric(palette = "plasma", domain = WQ_Sal%>% dplyr::select(contains("Sal")) %>% unlist() %>% range(na.rm = T))) %>% cols_align(align = c("center"), columns = everything()))
#
#
##WORKING BELOW
#
