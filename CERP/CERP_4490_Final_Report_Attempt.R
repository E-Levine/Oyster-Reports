###CERP 4490 Final Report - Attempting to duplicate SAS output
##2019-2023 Data
...
(Counts_df <- Counts %>% 
    mutate(Live = case_when(Live_raw >= 0 & Date <= '2019-01-01' & (Season == 'Spr'|Season == 'Fal') ~ Live_raw,
           ...
           Dead = case_when(Dead_raw >= 0 & Date <= '2019-01-01' & (Season == 'Spr'|Season == 'Fal') ~ Dead_raw,
                            ...
                            TRUE ~ DeadQtr*4),
           Total = Live + Dead) %>%
    subset((Date >= as.Date('2019-01-01', format = "%Y-%m-%d") & Date <= as.Date('2024-01-01', format = "%Y-%m-%d")) & (Site == "SL-C" | Site == "SL-N" | Site == "SL-S" | Site == "LX-N" | Site == "LX-S") &
