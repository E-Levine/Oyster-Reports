...
```{r SedimentFigures}
#Creating Sediment figures per section for better customization
lv5 <- list("AB", "SS")
sediment_plot_list <- list()

for (i in seq_along(lv5)) {
  df_source <- get(paste0(lv5[i], "_SedimentStats")) 
  max_upper <- max(ungroup(df_source) %>%  
                     dplyr::select(Upper), na.rm = TRUE)

  sediment_plot_list[[i]] <- df_source %>% 
...
ggarrange(sediment_plot_list[[1]] + theme(plot.margin = unit(c(0,0.2,0,0.1), "lines")),
          nrow = 1, ncol = 1)
