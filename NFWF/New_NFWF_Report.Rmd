...
for (i in seq_along(lv1)) {
  fig_name <- paste0("Recruit_", lv1[i], "_fig")
  df_source <- get(paste0(lv1[i], "_RecruitStats")) 
  max_upper <- max(ungroup(df_source) %>%  
                     dplyr::select(Upper), na.rm = TRUE)
# Add a 5% buffer on top of the max_upper
  buffer_percentage <- 1.05
  buffered_upper_limit <- ceiling(max_upper * buffer_percentage)
