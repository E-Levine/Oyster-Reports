...
RECode = "SS"  # Report Estuary Code. Use 2-letter code in Primary Keys
REName = "Suwannee Sound"  # Report Estuary Name. Use long name you want to appear in report
...
WQText <- R_WQStats %>%
  filter(Plot_Date >= RStart & Plot_Date < REnd)
...
Preliminary recruitment data is available through the end of `r format(max(RText$AnalysisDate), "%B %Y")`. Recruitment rates ranged from `r round(min(RText$RecMean), 1)` to `r round(max(RText$RecMean), 1)` spat per shell in the last quarter. The maximum rate of `r round(max(RText$RecMean), 1)` was observed at Station `r RText$StationNumber[which.max(RText$RecMean)]` during sampling in `r format(RText$AnalysisDate[which.max(RText$RecMean)], "%B %Y")`. 

<!-- Preliminary sediment trap data is available through the end of `r format(max(STText$AnalysisDate), "%B %Y")`. Sediment rates ranged from `r round(min(STText$SedMean), 1)` to `r round(max(STText$SedMean), 1)` grams (dry weight) of sediment in the last quarter. The maximum rate of `r round(max(STText$SedMean), 1)` was observed at Station `r STText$StationNumber[which.max(STText$SedMean)]` during sampling in `r format(STText$AnalysisDate[which.max(STText$SedMean)], "%B %Y")`.  -->

Preliminary water quality data is available through the end of `r format(max(WQText$AnalysisDate), "%B %Y")`. Water temperature, salinity, dissolved oxygen concentration, pH, water depth and Secchi depth were recorded during sampling (Figs. 10 – 14). Over the past quarter, water temperatures ranged from `r round(min(WQText$TempMean), 1)` to `r round(max(WQText$TempMean), 1)`°C; Salinities ranged from `r round(min(WQText$SalMean), 1)` to `r round(max(WQText$SalMean), 1)`; dissolved oxygen  ranged from `r round(min(WQText$DOMean), 1)` to `r round(max(WQText$DOMean), 1)` mg/L; pH values ranged from `r round(min(WQText$pHMean), 1)` to `r round(max(WQText$pHMean), 1)`.
...
#
##North
RW_fig2 <- RStats %>% filter(SectionName == "N") %>%  #Filter to one section
  ...
  scale_y_continuous("Spat / Shell", expand = c(0, 0),
                     limits = c(0,
                                2*ceiling(max(ungroup(RStats) %>% filter(SectionName == "N") %>% dplyr::select(Upper), na.rm = T)/2)), #Round to next highest even number for limits and breaks
                     breaks = seq(0, 2*ceiling(max(ungroup(RStats) %>% filter(SectionName == "N") %>% dplyr::select(Upper), na.rm = T)/2), by = 3))+ #Change by = # to adjust number of breaks
  ggtitle("North") + BaseForm + Station_fill
#
...
RC_fig2 <- RStats %>% filter(SectionName == "C") %>%  #Filter to one section
  ...
  scale_y_continuous("Spat / Shell", expand = c(0, 0),
                     ...
                     breaks = seq(0, 2*ceiling(max(ungroup(RStats) %>% filter(SectionName == "C") %>% dplyr::select(Upper), na.rm = T)/2), by = 20))+ #Change by = # to adjust number of breaks
  ggtitle("Central")+ BaseForm + Station_fill
#

##South
RE_fig2 <- RStats %>% filter(SectionName == "S") %>%  #Filter to one section
  ggplot(aes(x = as.POSIXct(Plot_Date), y = RecMean, fill = StationNumber))+ #Set base data to present
  geom_bar(stat = "identity", position = "dodge",  color = "black", width = 1565200) + #Add bars
  geom_errorbar(aes(as.POSIXct(Plot_Date), ymin = RecMean, ymax = Upper), position = position_dodge(), width = 1565200)+ #Add error bars
  scale_x_datetime("", expand = c(0.005, 0), #Set axis title and remove spacing
  ...
  scale_y_continuous("Spat / Shell", expand = c(0, 0),
                     limits = c(0,
                                2*ceiling(max(ungroup(RStats) %>% filter(SectionName == "S") %>% dplyr::select(Upper), na.rm = T)/2)), #Round to next highest even number for limits and breaks
                     breaks = seq(0, 2*ceiling(max(ungroup(RStats) %>% filter(SectionName == "S") %>% dplyr::select(Upper), na.rm = T)/2), by = 3))+ #Change by = # to adjust number of breaks
  ggtitle("South")+ BaseForm + Station_fill
```

```{r, warning = FALSE, message = FALSE, fig.cap = "Figure 3. Monthly mean number (± S.D.) of spat (oyster recruits) per shell collected at stations in the North, Central, and South sections of Suwannee Sound over the past fifteen months (five quarters). "}
...
#
##North
TW_fig3 <- R_WQStats %>% filter(SectionName == "N") %>%  #Filter to one section
  ...
  ggtitle("North") + BaseForm + Station_color + Station_fill + Station_shape
#
...
##South
TE_fig3 <- R_WQStats %>% filter(SectionName == "S") %>%  #Filter to one section
  ggplot(aes(x = as.POSIXct(Plot_Date), y = TempMean, color = StationNumber, group = StationNumber))+ #Set base data to present
  geom_line(aes(group=StationNumber), color = "black") + #Add connecting lines
  geom_point(aes(shape = StationNumber, fill = StationNumber), color = "black", size = 2.5) +  #Then add points on top. Fill = inside color of shape; Color = outside line of shape
  scale_x_datetime("", expand = c(0.035, 0), #Set axis title and remove spacing
  ...
  scale_y_continuous("Temperature (°C)", expand = c(0, 0),
                     limits = c(0, 40), pretty_breaks())+
  ggtitle("South") + BaseForm + Station_color + Station_fill + Station_shape


```

```{r, warning = FALSE, message = FALSE, fig.cap = "Figure 4. Monthly mean temperature at recruitment and sedimentation monitoring stations in the North, Central, and South sections of Suwannee Sound over the past fifteen months (five quarters). "}
...
#
##North
SW_fig4 <-  R_WQStats %>% filter(SectionName == "N") %>%  #Filter to one section
...
##South
SE_fig4 <-  R_WQStats %>% filter(SectionName == "S") %>%  #Filter to one section
  ggplot(aes(x = as.POSIXct(Plot_Date), y = SalMean, color = StationNumber, group = StationNumber))+ #Set base data to present
  geom_line(aes(group=StationNumber), color = "black") + #Add connecting lines
  geom_point(aes(shape = StationNumber, fill = StationNumber), color = "black", size = 2.5) +  #Then add points on top. Fill = inside color of shape; Color = outside line of shape
  scale_x_datetime("", expand = c(0.035, 0), #Set axis title and remove spacing
...
```{r, warning = FALSE, message = FALSE, fig.cap = "Figure 5. Monthly mean salinity at recruitment and sedimentation monitoring stations in the North, Central, and South sections of Suwannee Sound over the past fifteen months (five quarters). "}
...
DW_fig5 <- R_WQStats %>% filter(Estuary == RECode) %>%
  filter(SectionName == "N") %>%  #Filter to one section
...
DE_fig5 <- R_WQStats %>% filter(Estuary == RECode) %>%
  filter(SectionName == "S") %>%  #Filter to one section
  ggplot(aes(x = as.POSIXct(Plot_Date), y = DOMean, color = StationNumber, group = StationNumber))+ #Set base data to present
  geom_line(aes(group=StationNumber), color = "black") + #Add connecting lines
  geom_point(aes(shape = StationNumber, fill = StationNumber), color = "black", size = 2.5) +  #Then add points on top. Fill = inside color of shape; Color = outside line of shape
  scale_x_datetime("", expand = c(0.035, 0), #Set axis title and remove spacing
...
```{r, warning = FALSE, message = FALSE, fig.cap = "Figure 6. Monthly mean dissolved oxygen (DO) concentration at recruitment and sedimentation monitoring stations in the North, Central, and South sections of Suwannee Sound over the past fifteen months (five quarters). "}
...
#
##North
pW_fig6 <- R_WQStats %>% filter(SectionName == "N") %>%  #Filter to one section
...
##South
pE_fig6 <- R_WQStats %>% filter(SectionName == "S") %>%  #Filter to one section
  ggplot(aes(x = as.POSIXct(Plot_Date), y = pHMean, color = StationNumber, group = StationNumber))+ #Set base data to present
  geom_line(aes(group=StationNumber), color = "black") + #Add connecting lines
  geom_point(aes(shape = StationNumber, fill = StationNumber), color = "black", size = 2.5) +  #Then add points on top. Fill = inside color of shape; Color = outside line of shape
  scale_x_datetime("", expand = c(0.035, 0), #Set axis title and remove spacing
...
```{r, warning = FALSE, message = FALSE, fig.cap = "Figure 7. Monthly mean pH at recruitment and sedimentation monitoring stations in the North, Central, and South sections of Suwannee Sound over the past fifteen months (five quarters). "}
