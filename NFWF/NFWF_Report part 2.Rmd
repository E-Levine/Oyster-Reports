...
hsdbShellBudgetQuadrat <- tbl(con,in_schema("hsdb", "SampleEventWQ")) %>%
  collect() %>%
  filter(substring(SampleEventID,1,2) == RECode)

dboShellBudgetQuadrat <- tbl(con,in_schema("hsdb", "SampleEventWQ")) %>%
  collect() %>%
  filter(substring(SampleEventID,1,2) == RECode)

hsdbShellBudgetSH <- tbl(con,in_schema("hsdb", "SampleEventWQ")) %>%
  collect() %>%
  filter(substring(SampleEventID,1,2) == RECode)

dboShellBudgetSH <- tbl(con,in_schema("hsdb", "SampleEventWQ")) %>%
  collect() %>%
  filter(substring(SampleEventID,1,2) == RECode)
