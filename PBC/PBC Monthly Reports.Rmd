...
WaterQual <- read_excel("//fwc-spfs1/FishBio/Molluscs/Oysters/Palm Beach County/Data/PBCWaterQuality.xlsx", sheet = "RawData", skip = 2, col_names = TRUE, na = c("", "Z"), trim_ws = TRUE, .name_repair = "universal") %>% 
  filter(!grepl('Proof', Date)) %>%
  mutate(Date = as.Date(as.numeric(Date), origin = "1899-12-30")) %>% drop_na(Date)

Rcrt_df <- read_excel("//fwc-spfs1/FishBio/Molluscs/Oysters/Palm Beach County/Data/PBCRecruitment.xlsx", sheet = "Spat", skip = 2, col_names = TRUE, na = c("", "Z"), trim_ws = TRUE, .name_repair = "universal")  %>% drop_na(Ret.Date)

Dermo_df <- read_excel("//fwc-spfs1/FishBio/Molluscs/Oysters/Palm Beach County/Data/PBCReproDisease.xlsx", sheet = "Dermo", skip = 3, col_names = TRUE, na = c("", "Z"), trim_ws = TRUE, .name_repair = "universal") %>% drop_na(Date)

Sedi_df <- read_excel("//fwc-spfs1/FishBio/Molluscs/Oysters/Palm Beach County/Data/PBCSedimentTrap.xlsx", sheet = "Final Data", skip = 3, col_names = TRUE, na = c("", "Z", "z"), trim_ws = TRUE, .name_repair = "universal") %>% drop_na(Dep.Date)

Survey_df <- if(Month == "03" | Month == "09"){read_excel(paste0("//fwc-spfs1/FishBio/Molluscs/Oysters/Palm Beach County/Data/PBCSurvey",Survey, ".xlsx"), sheet = "Counts", skip = 3, col_names = TRUE, na = c("", "Z"), trim_ws = TRUE, .name_repair = "universal") %>% drop_na()} else {"Survey was not conducted"}
