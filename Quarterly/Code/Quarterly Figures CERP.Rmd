...
output: 
  word_document: 
    ...
    fig_caption: yes
params:
  Start_date: "2026-04-01" #Start date of reporting quarter
  End_date: "2026-06-30"   #End date of reporting quarter
---
...
Stations_cages <- c("SLC" = "St. Lucie-Central Station 2", "LXN" = "Loxahatchee-North Station 2", "CRE" = "Caloosahatchee River - Iona Cove", "CRW" = "Caloosahatchee River - Kitchel Key")
...
Cage_df <- readWorkbook("../Summary data/CERP_PBC_Summary_Data.xlsx", sheet = 'SH Mort Site', detectDates = TRUE, check.names = TRUE)
#Water Quality
...
WQ_Tem_s <-  readWorkbook("../Summary data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'Temp Station', detectDates = TRUE, check.names = TRUE)
WQ_pH_s <-  readWorkbook("../Summary data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'pH Station', detectDates = TRUE, check.names = TRUE)
WQ_DO_s <-  readWorkbook("../Summary data/CERP_PBC_WQ_Summary_Data.xlsx", sheet = 'DO Station', detectDates = TRUE, check.names = TRUE)
...
pH_East_2 <- rbind(pH_East_1 %>% dplyr::select(contains("SL")) %>% gather(SiteStation, pH, -SL_RetDate) %>% rename("RetDate" = SL_RetDate),
  ...
  mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("SLN", "SLS", "SLC", "LXN", "LXS")), Station = as.factor(substr(SiteStation, 4, 4)), Helper = case_when(is.na(pH) ~ "*", TRUE ~ NA)) 

pH_West <- WQ_pH_s %>% dplyr::select(contains("CR")) %>% filter(CRE_RetDate_pH >= End_date %m-% months(13)) %>% dplyr::select(-CRW_RetDate_pH) %>% rename("CR_RetDate" = CRE_RetDate_pH) %>% gather(SiteStation, pH, -CR_RetDate) %>% rename("RetDate" = CR_RetDate) %>% mutate(Site = factor(substr(SiteStation, 1, 3), levels = c("CRE", "CRW")), Station = as.factor(substr(SiteStation, 4, 4)), Helper = case_when(is.na(pH) ~ "*", TRUE ~ NA)) 
#
#
##Dissolved Oxygen
...
for(i in seq_along(East_sites)){
  ...
  df_temp <- Counts_East_2 %>% filter(grepl("Live", SiteStation) & grepl(Working_site, SiteStation) & (AnalysisDate > (End_date %m-% months(12)))) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE))
...
East_live_fig <- ggarrange(
  ...
  East_Live_list[[3]] + rremove("x.text") + rremove("x.title") + rremove("y.title") + rremove("legend"), #+ scale_y_continuous(limits = c(0, 1600), breaks = seq(0, 1600, by = 400), expand = c(0,0)),
...
for(i in seq_along(East_sites)){
  #
  Working_site <- paste(East_sites[i])
  df_temp <- Counts_East_2 %>% filter(grepl("DeadRatio", SiteStation) & grepl(Working_site, SiteStation) & (AnalysisDate > (End_date %m-% months(12)))) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE))
  df_temp$HelperLocation <- (ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10)/5
  max_upper <- ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10 #*1.05 adds 5% buffer to top if needed, *10 & /10 rounds up to nearest 0.1
  #
...
for(i in seq_along(East_sites)){
  #
  Working_site <- paste(East_sites[i])
  df_temp <- SH_East %>% filter(grepl(Working_site, SiteStation) & (AnalysisDate > (End_date %m-% months(12)))) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE))
  df_temp$HelperLocation <- (ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10)/5
  max_upper <- ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10 #*1.05 adds 5% buffer to top if needed, *10 & /10 rounds up to nearest 0.1
  #
...
for(i in seq_along(West_Sites)){
  ...
  df_temp <- Counts_West %>% filter(grepl("Live", SiteStation) & grepl(Working_site, SiteStation) & (AnalysisDate > (End_date %m-% months(12)))) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE))
  df_temp$HelperLocation <- (ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10)/5
  max_upper <- ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10 #*1.05 adds 5% buffer to top if needed, *10 & /10 rounds up to nearest 0.1
  #
...
for(i in seq_along(West_Sites)){
  #
  Working_site <- paste(West_Sites[i])
  df_temp <- Counts_West %>% filter(grepl("DeadRatio", SiteStation) & grepl(Working_site, SiteStation) & (AnalysisDate > (End_date %m-% months(12)))) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE))
  df_temp$HelperLocation <- (ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10)/5
  max_upper <- ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10 #*1.05 adds 5% buffer to top if needed, *10 & /10 rounds up to nearest 0.1
  #
...
```{r CRE SH Figure, fig.cap = "**Figure 6.**  Mean semi-annual shell height (± S.D.) of live oysters present at stations within the five east coast study sites. Asterisks indicate when sampling was not conducted at specific stations. Winter and summer surveys were only conducted at the Iona Cove and Bird Island stations. Please note differences in magnitude of y-axis among sites.", fig.height = 4.5, fig.width = 8, fig.fullwidth=TRUE}
#
West_SH_list <- list()
for(i in seq_along(West_Sites)){
  #
  Working_site <- paste(West_Sites[i])
  df_temp <- SH_West %>% filter(grepl(Working_site, SiteStation) & (AnalysisDate > (End_date %m-% months(12)))) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE))
  df_temp$HelperLocation <- (ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10)/5
  max_upper <- ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10 #*1.05 adds 5% buffer to top if needed, *10 & /10 rounds up to nearest 0.1
  #
...
for(i in seq_along(East_sites)){
  #
  Working_site <- paste(East_sites[i])
  df_temp <- Rcrt_East_2 %>% filter(grepl(Working_site, SiteStation)) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE)) %>% filter(RetDate >= (Start_date %m-% months(9)))
  df_temp$HelperLocation <- (ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10)/5
  max_upper <- ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10 #*1.05 adds 5% buffer to top if needed, *10 & /10 rounds up to nearest 0.1
  #
...
for(i in seq_along(West_Sites)){
  #
  Working_site <- paste(West_Sites[i])
  df_temp <- Rcrt_West %>% filter(grepl(Working_site, SiteStation)) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE)) %>% filter(RetDate >= (Start_date %m-% months(9)))
  df_temp$HelperLocation <- (ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10)/5
  max_upper <- ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10 #*1.05 adds 5% buffer to top if needed, *10 & /10 rounds up to nearest 0.1
  #
...
DermoP_East_2 %>%
  ...
  scale_x_date("Date", expand= c(0,0), date_breaks = "1 month", date_labels = "%b %Y", limits = c(End_date %m-% months(12) %m-% days(5), End_date %m-% days(5)))+ 
  Station_fill_E + Station_color_E + BaseForm + FacetForm + East_legend + guides(text = "none", color = "none")
```
...
DermoI_East_2 %>%
  ...
  geom_col(position = position_dodge(12), width = 7, color = "black") + 
  geom_errorbar(aes(ymin = Mean, ymax = Mean + SD, color = Station), position = position_dodge(12), width = 10) +
  geom_text(aes(y = 1, label = Helper, color = Station), position = position_dodge(12), size = 9)+
  facet_wrap(.~Site, nrow = 5, labeller = as_labeller(c("SLN" = "St. Lucie-North", "SLS" = "St. Lucie-South", "SLC" = "St. Lucie-Central", "LXN" = "Loxahatchee-North", "LXS" = "Loxahatchee-South"))) +
  scale_y_continuous("Dermo Infection Intensity", expand = c(0,0), limits = c(0, 5))+
  scale_x_date("Date", expand= c(0,0), date_breaks = "1 month", date_labels = "%b %Y", limits = c(End_date %m-% months(12) %m-% days(5), End_date %m-% days(5)))+
  Station_fill_E + Station_color_E + BaseForm + FacetForm + East_legend + theme(axis.title.y = element_text(margin = margin(r = 8))) + guides(text = "none", color = "none")
...
annotate_figure(DermoP_West_Fig, 
                ...
                bottom = text_grob("Date", family = "serif", size = 16.5, color = "black", vjust = 0.73))
...
DermoI_West_Fig <- ggarrange(
  DermoI_West %>% filter(Station == 1 | Station == 2) %>%
    ggplot(aes(RetDate, Mean, fill = Station)) + 
    geom_errorbar(aes(ymin = 0, ymax = Mean + SD, color = "black"), position = position_dodge(12), width = 10, size = 0.75) +
    geom_col(position = position_dodge(12), width = 7, color = "black") + 
    geom_text(aes(y = 1, label = Helper, color = Station), position = position_dodge(12), size = 9)+
    scale_y_continuous("Dermo Infection Intensity", expand = c(0,0), limits = c(0, 5))+
    scale_x_date("Date", expand= c(0,0), date_breaks = "1 month", date_labels = "%b %Y", limits = c(End_date %m-% months(12) %m-% days(5), End_date %m-% days(5)))+ 
...
```{r CERP Cage Mortality Figure, fig.cap = "**Figure 15.**  Mean monthly percent mortality (± S.D.) of oysters planted into open cages at Station 2 in St. Lucie-Central (top), Station 2 in Loxahatchee-North (2nd from top), Iona Cove in Caloosahatchee River-East (2nd from bottom) and Kitchel Key in Caloosahatchee River-West (bottom). Please note, oysetrs for the St. Lucie-Central cage were collected from station 1.", fig.height = 10, fig.width = 8, fig.fullwidth=TRUE}
Cage_Mort %>%
...
```{r CERP Cage SH Figure, fig.cap = "**Figure 16.**  Mean monthly deployed (hatched bars) and retrieved (solid bars) shell height (± S.D.) of oysters planted into open cages at Station 2 in St. Lucie-Central (top), Station 2 in Loxahatchee-North (2nd from top), Iona Cove in Caloosahatchee River-East (2nd from bottom) and Kitchel Key in Caloosahatchee River-West (bottom). Please note, oysetrs for the St. Lucie-Central cage were collected from station 1.", fig.height = 10, fig.width = 8, fig.fullwidth=TRUE}
Cage_SH %>% 
...
pH_East_2 %>%
  ...
  geom_point(aes(fill = Station), size = 2.75) + geom_line(aes(color = Station), linewidth = 0.7) +
  geom_text(aes(y = 7.5, label = Helper), size = 8.5, legend = FALSE)+
  facet_wrap(.~Site, nrow = 5, labeller = as_labeller(c("SLN" = "St. Lucie-North", "SLS" = "St. Lucie-South", "SLC" = "St. Lucie-Central", "LXN" = "Loxahatchee-North", "LXS" = "Loxahatchee-South"))) +
