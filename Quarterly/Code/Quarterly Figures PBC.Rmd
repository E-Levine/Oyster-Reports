...
for(i in seq_along(PBC_sites)){
  #
  Working_site <- paste(PBC_sites[i])
  df_temp <- Recruits %>% filter(grepl(Working_site, SiteStation)) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE)) %>% filter(RetDate >= (Start_date %m-% months(9)))
  df_temp$HelperLocation <- (ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10)/5
  max_upper <- ceiling((max(df_temp$Upper, na.rm = T)*1.05)*10)/10 #*1.05 adds 5% buffer to top if needed, *10 & /10 rounds up to nearest 0.1
  #
...
for(i in seq_along(PBC_sites)){
  #
  Working_site <- paste(PBC_sites[i])
  df_temp <- Sed_sta_rate %>% filter(grepl(Working_site, Site)) %>% rowwise() %>% mutate(Upper = sum(Mean, SD, na.rm = TRUE)) %>% filter(RetDate >= (Start_date %m-% months(9)))
  ...
  Sedi_list[[i]] <- df_temp %>% ggplot(aes(RetDate, Mean, fill = Station, color = Station))+ 
    ...
    geom_col_pattern(aes(pattern = ifelse(format(RetDate, "%Y-%m") == "2025-05" | format(RetDate, "%Y-%m") == "2025-09", "stripe", NA)), pattern_density = 0.4, pattern_fill = "white", position = position_dodge(13), width = 10, color = "black", show.legend = FALSE)+
...
for(i in seq_along(Sediment_list)){
  ...
  Posi_list[[i]] <- df_temp %>% ggplot(aes(RetDate, Mean, fill = Position))+ 
    ...
    #Adding pattern to indicate estimated values:
    geom_col_pattern(aes(pattern = ifelse(format(RetDate, "%Y-%m") == "2025-05" | format(RetDate, "%Y-%m") == "2025-09", "stripe", NA)), pattern_density = 0.4, pattern_fill = "white", position = position_dodge(15), width = 13, color = "black", show.legend = FALSE)+
